...
\item A cross-lingual retrieval system enabling questions in ten Indian languages to retrieve relevant passages from an English corpus, with measured quality across four languages---three retrieving correctly at 0.612--0.637 cosine similarity, two queries failing---and evidence that the failure is lexical rather than a matter of corpus coverage, since it survives a fivefold enlargement of the corpus unchanged.
...
Retrieval was evaluated with \textbf{five queries spanning four Indian languages}---Hindi, Marathi, Bengali and Tamil---against the English-medium portion of the corpus (Table~\ref{tab:crosslingual}). The queries themselves are committed alongside the benchmark rather than only their results, so the experiment can be repeated on any corpus state.

Three queries retrieved the correct passage at rank one and two did not, so the four languages and the three successes describe different quantities and are reported separately throughout: coverage is four languages, and the similarity range of 0.612--0.637 characterizes only the successful retrievals. Hindi appears twice because the same language succeeded on one topic and failed on another, which is the point made below: the failure belongs to the query term, not to the language.

Success is judged on the top passage alone. An earlier scoring pass asked only whether an expected term occurred anywhere in the top five, and by that measure all five queries succeeded; inspection showed that for both photosynthesis queries the on-topic passage sat at rank three or four, beneath chapters on the reflection and refraction of light. Retrieval that ranks optics above biology has failed the student whatever lies below it, and a metric that cannot say so is worse than none. The corpus grew fivefold between an earlier evaluation and this one---30 books and 9{,}722 passages to 153 and 42{,}995---and the outcome did not move: the same three queries succeed, the same two fail. Enlarging the corpus is not the remedy, which is what identifies the cause as lexical.

\begin{table}[htbp]
\caption{Cross-Lingual Retrieval Results, 153-book corpus. A query is correct only if the \emph{top} passage is on topic; where it is not, the rank of the first on-topic passage is given.}
\label{tab:crosslingual}
\centering
\small
\begin{tabular}{@{}llrl@{}}
\toprule
\textbf{Language} & \textbf{Topic} & \textbf{Sim.} & \textbf{Result} \\
\midrule
Marathi & Electric current & 0.637 & Correct \\
Hindi & Human eye focusing & 0.636 & Correct \\
Bengali & Heart function & 0.612 & Correct \\
Hindi & Photosynthesis & 0.558 & \textbf{Failed} (rank 3) \\
Tamil & Photosynthesis & 0.513 & \textbf{Failed} (rank 4) \\
\bottomrule
\end{tabular}
\end{table}

The two failures share a common cause: compound scientific terminology. Both the Hindi and Tamil words for ``photosynthesis'' begin with the word for ``light,'' causing retrieval to match the optics chapter rather than biology.
...
\textbf{Limitations.} (1)~The grounding threshold is preliminarily calibrated on only four answers due to manual verification limits; this statistically small sample size requires expansion. (2)~Cross-lingual retrieval is evaluated with five queries across four of the ten supported languages, a sample small enough that the three-of-five success rate carries wide error bars. (3)~27 legacy-font Hindi books remain unprocessable. (4)~Memory measurements have run-to-run variance of up to 414~MB.
