...
Table~\ref{tab:comparison} compares this work with the systems an Indian student is most likely to encounter: Khanmigo, Khan Academy's GPT-4 tutor; BYJU'S, the largest Indian ed-tech platform; Bhashini, the Government of India's national language-technology mission; and a general-purpose assistant used directly.
...
Bhashini is dashed rather than marked absent on several rows because it is language infrastructure---translation, speech and transliteration services---and not a tutor, so corpus and retrieval questions do not apply to it. It is included because it is the most capable Indian-language system in public deployment, and because its 22 languages exceed ours: a platform claiming multilingual reach should be compared against the strongest such system rather than only against tutors.
