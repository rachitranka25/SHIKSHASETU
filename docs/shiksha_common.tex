...
Bhashini is dashed rather than marked absent on several rows because it is language infrastructure---translation, speech and transliteration services---and not a tutor, so corpus and retrieval questions do not apply to it. It is included because it is the most capable Indian-language system in public deployment, and because its 22 languages~\cite{b22} exceed ours: a platform claiming multilingual reach should be compared against the strongest such system rather than only against tutors.
...
\begin{table}[htbp]
\caption{Cross-lingual retrieval: 22 queries over 11 languages, against the English-medium corpus. Correct = on-topic passage at rank~1.}
\label{tab:crosslingual}
\centering
\small
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Configuration} & \textbf{Eye} & \textbf{Photo.} & \textbf{Overall} \\
\midrule
BM25 (lexical)            & 0/11 & 0/11 & 0/22 \phantom{0}(0\%) \\
BM25 + rewrite            & 6/11 & 4/11 & 10/22 (45\%) \\
Dense (BGE-M3)            & \textbf{9/11} & 2/11 & 11/22 (50\%) \\
Dense + rewrite           & 6/11 & \textbf{8/11} & 14/22 (64\%) \\
\textbf{Pooled} (deployed) & 8/11 & \textbf{8/11} & \textbf{16/22 (73\%)} \\
\bottomrule
\end{tabular}
\end{table}
...
\ifconf\else  % journal only -- Grade-level classification: a retrieval detail, not a result
