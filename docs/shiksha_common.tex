% Springer LNCS builds set \springertrue before including this file. The
% IEEE builds do not define the flag at all, so it defaults to false here
% and nothing about them changes.
\makeatletter
\@ifundefined{ifspringer}{\newif\ifspringer\springerfalse}{}
\makeatother
\ifspringer\else\IEEEoverridecommandlockouts\fi
\usepackage{cite}
\usepackage{amsmath,amssymb,amsfonts}
\usepackage{graphicx}
\usepackage{textcomp}
\usepackage{xcolor}
\usepackage{booktabs}
% hidelinks: without it hyperref draws a coloured rectangle around every
% citation and cross-reference. It is invisible on screen in some viewers
% and prints as a green box, which is not what a submitted paper should
% look like.
\usepackage[hidelinks]{hyperref}
\usepackage{url}
\usepackage{multirow}
\usepackage{tikz}
\usetikzlibrary{positioning,arrows.meta,fit,backgrounds,shapes.geometric}
% Character protrusion and font expansion. This removes no content and adds
% no rules: it lets the engine break lines slightly better, which recovers
% a page across a paper this dense and reads marginally better while doing
% it. Under XeTeX protrusion is the part that works, so expansion is off.
\usepackage[protrusion=true,expansion=false,final]{microtype}
% LNCS sets a wider measure than an IEEE column, and several paragraphs here
% contain long unbreakable tokens -- \texttt{vector(1024)}, URLs, citation
% runs -- that cannot be hyphenated. Without a little stretch the engine
% overshoots the margin rather than loosening the line.
\ifspringer\emergencystretch=2em\fi
% llncs inherits \flushbottom, which stretches the space between items to make
% every page end at the same depth. On a page carrying two floats and little
% text that stretch lands between the floats and reads as a hole. \raggedbottom
% keeps the float separation at its natural size and leaves the slack at the
% foot, where it is ordinary.
\ifspringer\raggedbottom\fi
\makeatletter
\renewcommand{\fps@figure}{H}\renewcommand{\fps@table}{htbp}
\makeatother
\ifspringer
  \pgfkeys{/tikz/.cd, every picture/.append style={scale=0.88, every node/.append style={scale=0.88}}}
\fi
% A float may not cross a section boundary. Without this the request-path
% figure drifted into Related Work in one build and sat with System
% Architecture in another, so the same figure introduced itself a section
% early depending on which version you read.
% [section] puts a float barrier before every section. In the two-column IEEE
% layout that stops a figure drifting into the wrong section. In the Springer
% build it ends a page early whenever a float cannot fit before the barrier,
% which is a visible hole, so the barrier is IEEE-only.
\ifspringer\usepackage{placeins}\else\usepackage[section]{placeins}\fi
% Discourage a paragraph's first line from being stranded at the foot of a
% column, or its last line at the head of the next. LaTeX's defaults (150)
% treat these as mild; in a two-column layout they are what makes a page
% look broken.
\widowpenalty=10000
\clubpenalty=10000
\displaywidowpenalty=10000
\usepackage{float}
% Floats sat further from the text than IEEE needs.
\ifspringer\else
\setlength{\textfloatsep}{7pt plus 2pt minus 2pt}
\setlength{\abovecaptionskip}{4pt}
\fi
% Room for more than the default two floats per page, and permission to
% give most of a page to them, so none is deferred until it is lost.
\ifspringer\else
\setcounter{topnumber}{2}\setcounter{bottomnumber}{1}\setcounter{totalnumber}{3}
\renewcommand{\topfraction}{0.92}\renewcommand{\bottomfraction}{0.55}
\renewcommand{\textfraction}{0.07}\renewcommand{\floatpagefraction}{0.75}
\setlength{\floatsep}{7pt plus 2pt minus 2pt}
\setlength{\intextsep}{7pt plus 2pt minus 2pt}
\fi
\usepackage{pgfplots}
% Two hues, checked with a colour-vision validator before use, not by eye:
% worst-pair CVD dE 24.7 and normal-vision dE 33.6, both clear of the floors,
% and each above 3:1 against the page. Colour also carries meaning in Fig. 1,
% where it separates what runs on the machine from what does not.
\definecolor{sBlue}{HTML}{2A78D6}
\definecolor{sOrange}{HTML}{EB6834}
\pgfplotsset{compat=1.18}
% One visual system for every plot: no chartjunk, left-hand axes only, the same
% two greys throughout, and figures that survive being printed in black.
\pgfplotsset{
  shiksha/.style={
    width=\columnwidth, height=4.4cm,
    axis lines*=left, tick align=outside, tick pos=left,
    ymajorgrids, grid style={black!10, line width=0.3pt},
    label style={font=\scriptsize}, tick label style={font=\scriptsize},
    legend style={font=\scriptsize, draw=none, fill=none},
    every axis title/.style={font=\scriptsize, above=1mm},
  },
  shikshabar/.style={
    shiksha, ybar, bar width=11pt,
    every axis plot/.append style={fill=sBlue!75, draw=sBlue!95, line width=0.5pt},
    nodes near coords, nodes near coords style={font=\tiny, black!65},
  },
}
\def\BibTeX{{\rm B\kern-.05em{\sc i\kern-.025em b}\kern-.08em
    T\kern-.1667em\lower.7ex\hbox{E}\kern-.125emX}}
% Times with real bold and italic. tgtermes/tgcursor are pdfTeX font packages;
% under XeTeX they left every \textbf and \textit rendering as LMRoman-Regular,
% so the compiled PDF had no bold or italic anywhere. fontspec loads the same
% TeX Gyre faces with their bold and italic variants present.
\usepackage{fontspec}
\setmainfont{texgyretermes}[
  Extension      = .otf,
  UprightFont    = *-regular,
  BoldFont       = *-bold,
  ItalicFont     = *-italic,
  BoldItalicFont = *-bolditalic ]
\setmonofont{texgyrecursor}[
  Extension   = .otf,
  UprightFont = *-regular,
  BoldFont    = *-bold,
  ItalicFont  = *-italic ]

\begin{document}

\title{Shiksha Setu: A Measurement-Driven, Local-Retrieval AI Tutoring Platform for Multilingual Indian Education}

% All five authors share one department, so IEEEtran's shared-affiliation form
% is both the correct one and the compact one -- five separate blocks would
% wrap to a third row and cost most of a column on page one.
\ifspringer
% llncs prints "Title Suppressed Due to Excessive Length" on every recto
% page unless a short form is supplied, and truncates the author list on
% the verso unless \authorrunning is given.
\titlerunning{Shiksha Setu: Local-Retrieval AI Tutoring for Indian Education}
\authorrunning{R. Ranka et al.}
\author{Rachit Ranka \and Aaditya Saini \and Priyanka Devi \and Yogesh Kohli \and Ruhani Mehta}
\institute{Department of Computer Science Engineering, Chandigarh University,\\
Mohali, India\\
\email{rankarachit5@gmail.com, aadityasaini899@gmail.com,}\\
\email{pjangra9105@gmail.com, ykohli2005@gmail.com,}\\
\email{ruhanimehta019@gmail.com}}
\else
\author{\IEEEauthorblockN{Rachit Ranka, Aaditya Saini, Priyanka Devi, Yogesh Kohli, Ruhani}
\IEEEauthorblockA{\textit{Department of Computer Science Engineering} \\
\textit{Chandigarh University}, Mohali 140413, Punjab, India \\
rankarachit5@gmail.com, aadityasaini899@gmail.com, pjangra9105@gmail.com, \\
ykohli2005@gmail.com, ruhanimehta019@gmail.com}
}
\fi

\maketitle

\begin{abstract}
\ifconf
Existing AI tutoring systems depend on cloud infrastructure, operate primarily in English, and are not aligned to a national curriculum. Shiksha Setu answers all three. Its corpus, the embeddings over it, and the whole of retrieval sit on the user's own machine; generation stays a hosted call, and the paper does not claim to be offline. The retrieval results come from 155 ingested textbooks spanning classes~1--12, or 43{,}621 indexed passages; the grounding results predate two recovered books and are reported against the state they used. A five-configuration ablation over 77 queries in eleven Indian languages, run against an English-medium corpus, is where the cross-lingual gap shows most plainly: a lexical baseline retrieves \emph{nothing at all} for every one of them, since a Devanagari or Perso-Arabic query shares no tokens with an English index. Embedding the question directly recovers 47 of 77. Rewriting it into English first recovers 66. Those nineteen queries are where the improvement comes from; the pooling the system deploys adds one more. A grounding score separates answers written from their retrieved passages from answers that drifted, by 0.360 in cosine similarity over 256 pairs. The threshold shipped before this work admitted 35 of 240 negatives. Half precision halves the embedding footprint to 1{,}083~MB, which is what puts it on a 4~GB machine. Ingestion catches something subtler: legacy Devanagari fonts that render 66\% of Hindi textbooks as Latin gibberish.
\else
Existing AI tutoring systems depend on cloud infrastructure, operate primarily in English, and are not aligned to a national curriculum. This paper presents Shiksha Setu, a local-retrieval tutoring platform for the Indian NCERT curriculum in which the corpus, its embeddings and the whole of retrieval are local. Generation is a hosted call, and the paper is explicit about which half is which, and does not claim to be offline. Retrieval is measured against 155 ingested textbooks spanning classes~1--12 (1{,}557 chapters, 43{,}621 indexed passages) and grounding against the 153-textbook state that preceded it, each result being reported with the corpus state it used. Retrieval is evaluated by a five-configuration ablation over 22 queries in eleven Indian languages: a lexical baseline retrieves \emph{nothing at all}, because a Devanagari or Perso-Arabic query shares no tokens with an English index. Dense retrieval alone reaches 11 of 22, query rewriting alone 14, and pooling both, the deployed configuration, at $16\pm1$. A grounding score separating answers written from their retrieved passages from answers that drifted is calibrated on 256 pairs, not four examples, moving the shipped threshold from 0.55, which admitted 35 of 240 negatives, to 0.715, which admits none. The 0.360 separation reproduces under character $n$-gram TF-IDF and a second encoder family, so it is a property of the answers and not of one embedding space. Half-precision inference halves the embedding footprint to 1{,}083~MB at a mean cosine fidelity of 0.999998, verified by kernel-level peak resident set, enabling deployment on 4~GB machines. The pipeline audits its own sources, detecting legacy Devanagari fonts that render 66\% of Hindi textbooks as Latin gibberish, and a security audit repairs twelve vulnerabilities including a safety pipeline that failed open.
\fi
\ifspringer
\keywords{Retrieval-Augmented Generation \and Multilingual Education \and Local-Retrieval AI \and Indian Languages \and Curriculum-Aligned Tutoring \and Accessibility \and NCERT}
\fi
\end{abstract}

\ifspringer\else
\begin{IEEEkeywords}
Retrieval-Augmented Generation, Multilingual Education, Local-Retrieval AI, Indian Languages, Curriculum-Aligned Tutoring, Accessibility, NCERT
\end{IEEEkeywords}
\fi

%===================================================================
\section{Introduction}
%===================================================================

India has 1.4 billion people and 22 scheduled languages~\cite{b24}, written in more than a dozen scripts. The National Council of Educational Research and Training (NCERT) publishes the curriculum that governs instruction for the 24.7 crore children enrolled in school~\cite{b30}, yet its textbooks exist only in English, Hindi, and Urdu. A Tamil-speaking student in rural Tamil Nadu, a Marathi-speaking student in Maharashtra, and a Bengali-speaking student in West Bengal all study the same curriculum, but none of them can read it in their mother tongue unless a human teacher translates it in real time.

Artificial intelligence can bridge this gap, but the dominant paradigm of cloud-hosted large language models introduces three structural failures for Indian education:

\textbf{Language exclusion.} Most educational AI systems operate in English. English is a first language for a negligible share of the population and a second or third language for roughly one Indian in ten~\cite{b24}, so for the great majority such systems are inaccessible at the point of need. \textbf{Infrastructure dependence.} Cloud solutions need consistent bandwidth and bill per query, which makes sustained use impractical in the towns and villages where most students are. \textbf{Pedagogical misalignment.} A general-purpose model has no notion of curriculum structure or grade-appropriate complexity, and a class~6 student asking about chemical reactions needs a different answer from a class~12 student on the same topic.

This paper presents Shiksha Setu, a platform that addresses the first and third failures and \emph{partially} the second, through a measurement-driven engineering approach.

The qualification is deliberate. Retrieval is entirely local. The corpus, its embeddings and the vector index never leave the machine, but generation is served by a hosted endpoint, so a question's text does leave it and does incur a per-query cost. The system therefore removes the dependency for the component that dominates storage and latency without removing it altogether. Local generation is configured and available (Section~\ref{sec:models}), but a language model cannot share a 4~GB machine with the embedder. The offline configuration and the 4~GB configuration are therefore two different systems. We use the term local-retrieval because local-first gets read as ``offline'', and this system answers nothing without a network. The measurements in this paper were taken in the hosted configuration. Every claim is backed by an experiment whose procedure, data, and result are reported, including experiments that produced wrong results before being corrected. The contributions are:

The first contribution is an ingestion pipeline covering all 559 NCERT textbooks. It detects the legacy-font corruption that renders 27 of 41 Hindi books unreadable, a defect we found only by measuring for it. The second is a cross-lingual retrieval system answering questions in eleven Indian languages from an English corpus; a controlled ablation over 77 queries shows that rewriting the question into English before embedding it retrieves correctly for 66 of them, against 47 for embedding it directly, while a lexical baseline retrieves none. Third, a memory strategy verified by kernel-level measurement brings the serving pipeline inside a 4~GB budget and corrects an earlier arithmetic estimate that understated consumption by 31\%. Last, a grounding metric detects when fluent, confident answers have drifted from their source material. Without measurement that failure is invisible. We calibrate the metric on 256 answer and passage pairs and compare it against three alternative estimators.\ifconf\else\ The journal version additionally reports akshara-level readability measurement for nine Brahmic scripts, which replaces Latin-alphabet syllable counting with script-appropriate segmentation, and a security audit documenting twelve vulnerabilities, all repaired, among them a safety pipeline that failed open because of an import error.\fi

%===================================================================
\section{Related Work}
%===================================================================

Lewis et al.~\cite{b2} introduced retrieval-augmented generation (RAG), since surveyed at length~\cite{b28}, establishing the paradigm of conditioning language model output on retrieved passages. Shiksha Setu adopts this architecture with a critical modification: a grade-level filter inserted between retrieval and generation, because the same topic at class~6 and class~12 requires different explanations. Karpukhin et al.~\cite{b3} found dense passage retrieval stronger than BM25. Ma et al.~\cite{b4} showed that query rewriting before embedding improves retrieval quality. The technique that resolves the romanized Hindi failure\ifconf\else\ of Section~\ref{sec:romanized}\fi. Nogueira and Cho~\cite{b5} established passage re-ranking with cross-encoder models. The BGE-Reranker component exists in the codebase but is not yet wired into the retrieval path.

Chen et al.~\cite{b6} presented BGE-M3, a multilingual embedding model supporting over 100 languages in a unified 1024-dimensional vector space. Its cross-lingual property, that semantically equivalent passages in different languages map to nearby vectors, allows a Hindi question to retrieve an English passage without explicit translation. Gala et al.~\cite{b7} developed IndicTrans2, which covers every scheduled language and serves as the translation backbone. Kakwani et al.~\cite{b1} released Indic corpora and the benchmarks for evaluating on them. Khanuja et al.~\cite{b8} trained MuRIL on transliterated text, directly relevant to the romanized Hindi failure\ifconf\else\ documented in Section~\ref{sec:romanized}\fi.

\ifshort\else  % six-page build: related work, not load-bearing
Recent literature categorizes multilingual retrieval into frameworks such as MultiRAG (direct retrieval) and tRAG (translation-RAG). The platform described in this work formally implements a tRAG architecture~\cite{b9}, rewriting queries to a high-resource language (English) before retrieval. This architectural choice unknowingly resolves a newly documented vulnerability in large language models: Output Language Drift~\cite{b10}. Studies show that when a multilingual RAG system processes a query in a target language but retrieves context in a dominant high-resource language, the model mixes languages and outputs the final answer in the high-resource language. The strict separation of the generation language from the delivery language via the translation layer elegantly neutralizes this cognitive drift.
\fi

The retrieval layer uses the HNSW graph structure proposed by Malkov and Yashunin~\cite{b11}, implemented through the pgvector extension for PostgreSQL. The index parameters ($m=16$, $ef\_construction=64$) balance build time against recall. The resulting search latency is reported in Fig.~\ref{fig:pipeline}, not here. Reimers and Gurevych~\cite{b12} established the sentence-level embedding formulation used to load BGE-M3.

% BEFORE SUBMITTING THE JOURNAL VERSION: once the conference paper is
% accepted, IEEE requires this version to declare it -- "A preliminary
% version of this work appeared in [ref]" -- and to cite it. 57% of this
% version is material the conference paper does not contain, which clears
% the usual 30% bar for an extension, but the declaration is still required
% and its absence is a desk-reject.
\ifconf\else  % journal only -- the conference version presents no accessibility contribution
Zorzi et al.~\cite{b13} found that extra-large letter spacing improved reading speed and halved errors in dyslexic children, the single typographic intervention with strong experimental support and the default accessibility control in the platform. Rello and Baeza-Yates~\cite{b14} and Kuster et al.~\cite{b15} found no reliable advantage for dyslexia-specific typefaces. Nag~\cite{b16} and Nag and Snowling~\cite{b17} demonstrated that akshara-based literacy develops differently from alphabetic literacy, motivating the akshara-level segmentation\ifconf\else\ described in Section~\ref{sec:accessibility}\fi.
\fi

\ifconf
Micikevicius et al.~\cite{b20} established the safety of half-precision arithmetic for neural network inference, providing the basis for the fp16 optimization that halves the BGE-M3 memory footprint.
\else
Kincaid et al.~\cite{b18} derived the Flesch-Kincaid grade-level formula defined on English syllable counts, which is why the platform refuses to compute it for Devanagari text. Xu et al.~\cite{b19} demonstrated that generic simplification routinely loses content, motivating the measure-and-keep-the-easier approach. Micikevicius et al.~\cite{b20} established the safety of half-precision arithmetic for neural network inference, providing the theoretical basis for the fp16 optimization that halves the BGE-M3 memory footprint.
\fi

\label{sec:comparison}
Table~\ref{tab:comparison} compares this work with the systems an Indian student is most likely to encounter: Khanmigo~\cite{b21}, Khan Academy's GPT-4 tutor; BYJU'S, the largest Indian ed-tech platform; and Bhashini~\cite{b22}, the Government of India's national language-technology mission.

The comparison is confined to \emph{architectural properties}. Every cell is a structural fact, a published vendor figure, or \emph{n/d} where none exists, and no row sets a measurement of ours against a number we could not obtain. Commercial products do not publish latency distributions, retrieval quality, or memory footprints. Estimating them would produce invented numbers dressed as measurements.

\begin{table}
\caption{Comparison with deployed systems. \emph{n/d}: not disclosed by the operator. Rows are architectural properties, not performance claims.}
\label{tab:comparison}
\centering
\footnotesize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Property} & \textbf{Khanmigo} & \textbf{BYJU'S} & \textbf{Bhashini} & \textbf{Shiksha Setu} \\
\midrule
NCERT-aligned corpus     & no  & yes & ---  & yes \\
Indian languages         & no  & n/d & 22   & 11 \\
Corpus held locally      & no  & no  & ---  & yes \\
Retrieval runs locally   & no  & no  & ---  & yes \\
Generation runs locally  & no  & no  & ---  & no  \\
Runs without a network   & no  & no  & no   & no  \\
Brahmic readability aid  & no  & no  & no   & yes \\
Per-answer grounding     & n/d & n/d & ---  & yes \\
Corpus corruption audit  & n/d & n/d & ---  & yes \\
Stated memory target     & n/d & n/d & n/d  & 4~GB \\
Tutoring function        & yes & yes & no   & yes \\
\bottomrule
\end{tabular}
\end{table}

\ifshort\else  % six-page build: the table's own caption carries the rule
Bhashini is dashed on several rows, not marked absent, because it is language infrastructure (translation, speech, transliteration) and not a tutor, so corpus and retrieval questions do not apply. It is included because its 22 languages~\cite{b22} exceed ours, and a platform claiming multilingual reach should be measured against the strongest such system.
\fi

Three rows are stated against our own interest: generation is not local, the system needs a network, and BYJU'S covers our curriculum at a scale we do not approach. What distinguishes this work is the rest. The corpus, index and retrieval are local, a grounding score accompanies every answer, and the pipeline audits its own sources instead of embedding whatever it extracts. We have found no comparable system reporting the last two, which is a claim about the published record and not about what those systems do internally.

%===================================================================
\section{System Architecture}
%===================================================================

A student's question traverses an eight-stage pipeline from input to response, six of them mandatory. The latencies below were taken on the development machine; what the platform requires is the 2{,}094~MB of Section~\ref{sec:memory}, not this hardware. Retrieval is local and its cost is a property of the index; generation is a hosted call and its cost is a property of the network.

\begin{figure}
\centering
\footnotesize
\begin{tikzpicture}[
  font=\footnotesize,
  stage/.style={draw, minimum height=5.2mm, inner xsep=2pt, align=center, font=\scriptsize},
  loc/.style  ={stage, fill=sBlue!12,  draw=sBlue!70, text width=27mm},
  net/.style  ={stage, fill=sOrange!16, draw=sOrange!80, text width=27mm},
  t/.style    ={font=\scriptsize, inner sep=1pt},
  fl/.style   ={-{Latex[length=1.3mm]}, thick},
  cross/.style={-{Latex[length=1.3mm]}, thick, densely dashed}]

% the machine boundary, which is what the figure exists to show
\draw[dashed, black!45, line width=0.5pt] (32mm,4mm) -- (32mm,-46mm);
\node[t, anchor=south, sBlue!85] at (14mm,4.5mm) {\textsc{this machine}};
\node[t, anchor=south, sOrange!90] at (52mm,4.5mm) {\textsc{network}};

\node[loc] (q)  at (14mm,  0mm)   {student's question};
\node[net] (rw) at (52mm, -8mm)   {1\, rewrite to English\\[-1pt]$\sim$0.5\,s, cached};
\node[loc] (em) at (14mm, -16mm)  {2\, embed query\\[-1pt]0.11\,s};
\node[loc] (rt) at (14mm, -25mm)  {3--5\, retrieve, score by\\[-1pt]class, retrieve again\\[-1pt]0.11\,s};
\node[net] (gn) at (52mm, -33mm)  {6\, generate answer\\[-1pt]4.2\,s};
\node[loc] (gr) at (14mm, -41mm)  {grounding check\\[-1pt]regenerate if ${<}0.715$};

\draw[cross, sOrange!85] (q)  -- (rw);
\draw[cross, sOrange!85] (rw) -- (em);
\draw[fl, sBlue!85]    (em) -- (rt);
\draw[cross, sOrange!85] (rt) -- (gn);
\draw[cross, sOrange!85] (gn) -- (gr);

\node[t, anchor=west, black!65] at (33mm,-20.5mm) {the corpus, its index};
\node[t, anchor=west, black!65] at (33mm,-23.5mm) {and all retrieval stay};
\node[t, anchor=west, black!65] at (33mm,-26.5mm) {on the left of this line};
\end{tikzpicture}
\caption{The mandatory request path. Medians over 15 warm requests, five questions run three times on an 8~GB Apple~M1. The dashed line is the machine boundary: the corpus and all retrieval stay left of it. Generation dominates and is least predictable, 4.2\,s median against a 95th percentile of 13.2\,s, because it is hosted. All of retrieval is 0.11\,s. The grounding check carries no latency because it was not measured separately.}
\label{fig:pipeline}
\end{figure}

\ifshort\else  % six-page build: the point is made with numbers elsewhere
The design separates the question's language from the answer's language. A student may type in romanized Hindi and receive a response in Tamil. This independence is critical because NCERT publishes in only three languages while the platform serves eleven. Translation is a serving-layer responsibility handled by IndicTrans2~\cite{b7}.
\fi

\label{sec:models}
The platform integrates twenty-eight model identifiers, of which \textbf{five} are exercised on the critical path and measured in this work. The remainder are configured alternatives that this evaluation does not characterize.\ifshort\ The five are BGE-M3 (568~M parameters, embeddings and grounding, the only one resident locally), and four hosted models: LLaMA-3.1-8B for generation and query rewriting, FLUX.1-dev for illustration, Nemotron-Nano-12B for vision OCR, and LLaMA-3.3-70B, compared and not adopted.\else\ Table~\ref{tab:models} separates the two, because a model stack presented as a single list would imply an empirical basis that only the first group has.\fi

\ifshort\else  % six-page build: replaced by the sentence below
\begin{table}
\caption{Model Stack. Upper block: measured on the critical path. Lower block: integrated but not on it. $\dagger$~denotes a hosted model; all others are resident locally. Reranking is the exception: it was evaluated and excluded on the result.}
\label{tab:models}
\centering
\footnotesize
\begin{tabular}{@{}llr@{}}
\toprule
\textbf{Role} & \textbf{Model} & \textbf{Params} \\
\midrule
Embeddings, grounding & BGE-M3 & 568M \\
Generation, rewriting & LLaMA-3.1-8B$\dagger$ & 8B \\
Illustration & FLUX.1-dev$\dagger$ & 12B \\
Vision OCR & Nemotron-Nano-12B$\dagger$ & 12B \\
Generation (compared) & LLaMA-3.3-70B$\dagger$ & 70B \\
\midrule
Translation & IndicTrans2-1B & 1B \\
Speech-to-Text & Whisper~V3~Turbo~\cite{b23} & 809M \\
Text-to-Speech & MMS-TTS (11 ckpts) & ${\sim}100$M \\
Curriculum Valid. & Gemma-2-2B-IT & 2B \\
Text Simplification & Qwen2.5-3B & 3B \\
Reranking & BGE-Reranker-v2-M3 & 568M \\
\bottomrule
\end{tabular}
\end{table}
\fi

\ifshort\else  % six-page build: the point is made with numbers elsewhere
Exactly one model is resident locally on the critical path. This is not an accident of packaging but the property that makes the memory budget of Section~\ref{sec:memory} achievable. Embedding must be local because every chunk in the corpus passes through it during ingestion, whereas generation, illustration and OCR are per-request and therefore hosted, leaving no language-model weights resident during serving.
\fi

BGE-M3 is used twice for distinct purposes: retrieving passages, and later scoring how far an answer strayed from them (Section~\ref{sec:grounding}). Both uses depend on the same property, cross-lingual embedding in a shared space, which is what permits a Devanagari passage and an English answer to be compared numerically at all.

\ifshort\else  % six-page build
Two integrated models are named but unusable in this deployment and are reported as such, not omitted. The local OCR path (GOT-OCR2) requires CUDA and cannot execute on Apple silicon. Its Python module additionally failed to import because \texttt{torchvision} was an undeclared dependency, which presented as a hardware limitation for some time before being diagnosed as a packaging one. The Tesseract fallback is installed without Devanagari training data, leaving it unable to read the script that most needs recovery (Section~\ref{sec:legacy}).
\fi

The choice of the 8B model over the 70B variant was driven by measurement\ifconf: on one fixed prompt at approximately 70 completion tokens against the same endpoint, LLaMA-3.1-8B returned in 0.9~s where LLaMA-3.3-70B took 17--125~s, highly variable\else\ (Table~\ref{tab:latency})\fi.

\ifconf\else  % journal only -- the figures that matter are in the sentence above
\begin{table}
\caption{Text generation model latency: one fixed prompt at approximately 70 completion tokens, against the same hosted endpoint. These are model-selection figures and are shorter than the end-to-end generation stage of Fig.~\ref{fig:pipeline}, which produces a full answer.}
\label{tab:latency}
\centering
\begin{tabular}{@{}lr@{}}
\toprule
\textbf{Model} & \textbf{Latency} \\
\midrule
meta/llama-3.1-8b-instruct & \textbf{0.9~s} \\
nvidia/nemotron-nano-9b-v2 & 2.5~s \\
meta/llama-3.3-70b-instruct & 17--125~s \\
nvidia/llama-3.1-nemotron-nano-8b-v1 & Timeout \\
\bottomrule
\end{tabular}
\end{table}
\fi

The persistence layer combines PostgreSQL~17 with pgvector for HNSW-indexed similarity search\ifshort. Response caching is tiered, and cache keys incorporate a SHA-256 hash of the authorization header so that one student's answer cannot be served to another.\else, Redis~7 for response caching, and SQLite for the third cache tier. The multi-tier architecture (L1: in-memory LRU $\to$ L2: Redis $\to$ L3: SQLite) serves read-heavy endpoints. Cache keys incorporate a SHA-256 hash of the authorization header to prevent cross-user leakage. The vector index uses cosine distance:\fi
\ifshort\else  % six-page build: the parameters are stated in the sentence above
\begin{verbatim}
CREATE INDEX idx_emb_hnsw_cosine
  ON embeddings USING hnsw
  (embedding vector_cosine_ops)
  WITH (m=16, ef_construction=64);
\end{verbatim}
\fi

An earlier migration stored embeddings as \texttt{double precision[]} where \texttt{vector(1024)} was required. Index creation became impossible and every search fell back to a sequential scan.

\ifconf  % the journal carries this at length in the Deployment section
\label{sec:hardware}
The 4~GB target is a claim about the machines schools own. Those are predominantly Windows desktops, while this work was developed on Apple hardware, so portability belongs inside the claim itself.

Device selection is resolved at load time in the order CUDA, Metal (MPS), CPU, and half precision is selected per device (Section~\ref{sec:precision}). Everything the platform needs to know about its host (installed memory, available memory, swap, peak resident set) is read through a single abstraction over \texttt{psutil}, which reports identically on Linux, Windows and macOS.

\ifshort\else  % six-page build: the claim stays, the detail goes
That abstraction replaced direct system calls that answered only on macOS: \texttt{sysctl hw.memsize} for installed memory, \texttt{memory\_pressure} and \texttt{sysctl vm.swapusage} for the ingestion memory guard, and \texttt{getrusage} for peak footprint. One call site was not inside an exception handler, so on Windows it raised rather than degrading; and the benchmark that substantiates the 4~GB figure imported the POSIX-only \texttt{resource} module at module scope, making it unrunnable on precisely the platform whose claim it exists to support.

Peak resident set is the one quantity with no portable source: \texttt{getrusage} is POSIX-only and Windows exposes a peak through the process handle. The abstraction reads whichever exists and returns a flag when it has had to fall back to current resident set, rather than presenting a smaller number as a high-water mark. A related unit bug is worth recording because it is silent: \texttt{ru\_maxrss} is bytes on macOS and kilobytes on Linux, so the same code reports figures a factor of 1024 apart on two POSIX systems.
\fi

\fi
All measurements in this paper were taken on an Apple~M1 with 8~GB of unified memory. We report no figures for hardware we did not run on.

%===================================================================
\section{Corpus Construction}
%===================================================================

NCERT encodes every textbook with a five-character code: \texttt{jesc1} represents Class~10, English medium, Science, book~1. The subject letters are not algorithmically derivable. Science is \texttt{sc} for classes~7--10 but \texttt{cu} (Curiosity) for class~6, and splits into \texttt{ph}, \texttt{ch}, \texttt{bo} at classes~11--12. The catalog is scraped from NCERT's textbook picker and cached locally.

The complete catalog contains \textbf{559 textbooks}: 209 in English, 191 in Hindi, and 159 in Urdu. The platform teaches from a deliberate subset of \textbf{263 books}, every English edition, plus the 54 subjects that exist only in Devanagari.

\ifshort\else  % six-page build: a scoping decision, not a result
\textbf{The 159 Urdu editions are excluded from the taught corpus entirely}, and are neither ingested nor retrieved from. They are translations of books already held in English, so including them would triple the storage for that curriculum while adding no content, and cross-lingual retrieval was observed to cite an Urdu chapter as the source of an English answer, a citation the student can neither read nor check. Urdu remains available as an \emph{answer} language, and appears as a query language in the retrieval evaluation of Section~\ref{sec:crossling}: what a student writes in and reads back is independent of which edition the system learned from.
\fi

The pipeline processes each book through five stages: ZIP download, per-chapter PDF extraction via PyMuPDF, text cleanup, semantic chunking (1200 characters, 200-character overlap, paragraph-boundary preference), BGE-M3 embedding (fp16), and storage in PostgreSQL. Each book is committed as a single database transaction for resumability. Download and embedding contend for no shared resource. One waits on a remote web server, the other saturates the GPU. Executed sequentially they idle in turn, so the next book's archive is fetched on a prefetch thread while the current book is embedded.

\ifspringer\else
We established the speedup by controlled comparison. The same three books (classes 6--8 \textit{Curiosity}) were ingested twice, once with prefetching disabled through \texttt{INGEST\_PREFETCH=0}, producing identical output: 193~s per book sequentially against 140~s with prefetching, a 1.38$\times$ speedup on the same hardware and corpus.
\fi

\ifshort\else  % six-page build: a methodological aside, not a result
We note that a naive comparison of figures taken under different conditions would have reported this backwards. An earlier sequential measurement of 97~s per book was recorded on a different, smaller book set. Read against the 140~s figure it appears to show prefetching making ingestion \textit{slower}. Only the paired measurement on identical inputs is informative, and unpaired throughput figures are excluded here for that reason.
\fi

\label{sec:legacy}
Two-thirds of the Hindi curriculum is rendered unreadable by every PDF text extractor tested. NCERT's older Hindi textbooks are typeset in \textbf{Walkman-Chanakya 905}, a legacy font mapping Devanagari glyphs onto ASCII codepoints with no \texttt{ToUnicode} table. Text extraction returns raw byte values. The analysis covers 41 of the 54 Devanagari-only books in the taught curriculum. The method samples a single chapter PDF per book, and for the remaining 13 books NCERT publishes no individual chapter files, returning HTTP~404 for chapters~1 through~3 at the documented URL scheme. Those 13 are absent because no sample could be retrieved for them, and nothing here depends on their contents (Table~\ref{tab:legacy}):

\begin{table}
\caption{Hindi Corpus Corruption Analysis}
\label{tab:legacy}
\centering
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Extraction Category} & \textbf{Books} & \textbf{Devanagari} \\
\midrule
Legacy font (Walkman-Chanakya) & \textbf{27} & 0.0\% \\
Unicode font (Kokila, Arya) & 14 & 97.9--100\% \\
\bottomrule
\end{tabular}
\end{table}

\ifshort\else  % six-page build: the corrected detector is described above
\textbf{The measurement that was inverted.} An earlier quality assessment counted broken Devanagari sequences and reported the 27 legacy-font books as the \textit{cleanest} in the corpus, because a detector searching for damaged Devanagari finds nothing in a book containing no Devanagari at all. 133 chunks of Latin gibberish entered the retrieval corpus before the error was caught. The corrected detection identifies Hindi books whose extracted text is overwhelmingly Latin.
\fi

\ifconf\else  % journal only -- OCR fidelity in mathematics: corpus detail
PyMuPDF's text layer is lossy on mathematical content. Class~10 Mathematics chapter~4 defines a quadratic as $ax^2 + bx + c,\ a \neq 0$. The text layer yields ``$ax2 + bx + c,\ a\text{~~}0$'': the superscript flattened and the inequality deleted. Pages requiring OCR are identified by image density (Table~\ref{tab:ocr}).

\begin{table}
\caption{Image Density as OCR Predictor}
\label{tab:ocr}
\centering
\begin{tabular}{@{}lr@{}}
\toprule
\textbf{Page Type} & \textbf{Images/1000 chars} \\
\midrule
Mathematics with equations & 30.8 \\
Science with figures & 5.9 \\
Prose with occasional figures & 2.0--4.0 \\
\bottomrule
\end{tabular}
\end{table}

Cloud OCR (Nemotron VL) correctly recovers symbols but introduces substitution errors worse for learners. The text layer loses glyphs. The vision model substitutes plausible but incorrect characters and hallucinates nonexistent headers.

\fi

\ifconf\else  % journal only -- the audit behind the corpus figures
\label{sec:deadends}
A book that cannot be ingested leaves a marker naming the reason, so the shortfall can be checked instead of assumed. There are 118 such markers, of which 110 fall inside the taught curriculum and account exactly for the gap between the 263 books taught and the 153 ingested, and the remaining 8 are Urdu or dual-medium editions outside it. Table~\ref{tab:deadends} groups them.

\begin{table}
\caption{Every book the pipeline could not ingest, by recorded reason. \emph{Origin} distinguishes a property of the published material from a property of the machine we ran on.}
\label{tab:deadends}
\centering
\footnotesize
\begin{tabular}{@{}lrl@{}}
\toprule
\textbf{Recorded reason} & \textbf{Books} & \textbf{Origin} \\
\midrule
No archive published (HTTP~404)        & 88 & source \\
Legacy font: no usable chapters        & 27 & source \\
Corrupt archive (\texttt{Bad CRC-32})  & 1  & source \\
\midrule
Disk exhausted mid-ingestion           & 1  & \textbf{ours} \\
\texttt{NaN} in an embedding vector    & 1  & \textbf{ours} \\
\midrule
\textbf{Total}                         & \textbf{118} & \\
\bottomrule
\end{tabular}
\end{table}

Two of the 118 are ours, and we would have reported the corpus shortfall as wholly a property of the source material had the markers not recorded a reason. One book exhausted the disk mid-ingestion. The other produced a vector containing \texttt{NaN}, which \texttt{pgvector} refused, and because a book commits in one transaction the whole book rolled back. Both causes are transient and neither is a statement about the book, yet \texttt{record\_dead\_end()} wrote the same permanent marker it writes for a corrupt archive. The two are recoverable by deleting their markers. One of them is the class~11 Physics text.

The \texttt{NaN} is the more interesting of the two. A single chunk embedding to \texttt{NaN} is enough to lose an entire textbook, and nothing upstream checks for it. The vector is written straight to a column that happens to reject it. Had the column been permissive the corpus would have gained a passage at infinite distance from every query, which fails quietly instead of loudly. We report it as a defect found by audit, not by test, because that is what it was.
\fi

\ifconf\else  % journal only -- the two design decisions the totals depend on
%===================================================================
\section{Two Decisions the Results Rest On}
\label{sec:decisions}
%===================================================================

Two implementation choices sit underneath every retrieval number in this paper, and both were arrived at by watching a simpler version fail.

Passages are 1{,}200-character windows with 200 characters of overlap, but the window is not taken at the character count. Each window is trimmed back to the last paragraph break within its final quarter, or failing that the last sentence end. A chunk cut mid-sentence embeds poorly and, because retrieved passages are shown to the student as the source of the answer, also reads as broken. The trim bounds how much text a boundary adjustment can discard while ensuring a chunk almost always ends where a human would end it.

\label{sec:grade}
The platform must decide which class's material answers a question, because the same topic at class~6 and class~12 requires different explanations. The decision is made from the retrieved passages themselves. Each class is scored as \textbf{the sum of its three strongest passages}, and the highest-scoring class wins. The rule is narrow, and it was reached by discarding the two obvious alternatives, each of which fails in a way we observed.

\emph{Summing every passage lets a class win on volume.} A chemistry question matched eight weak passages in class~1 picture books and three strong ones in the class~10 chapter that actually teaches it. Class~1 won. The corpus composition that makes this a real risk and not a hypothetical one is given in Section~\ref{sec:evalcorpus}.

\emph{Taking the single best passage fails in the opposite direction}, and so does averaging. A class with one passage at 0.66 beats a class with three at 0.60--0.62 because its mean is that lone passage. One high match can be a passing mention that happens to share wording. Three consistent ones mean the topic is taught there.

Capping the sum at three passages bounds what volume can contribute while still rewarding corroboration. The rule is not principled in any deeper sense. It is the smallest rule that resists both observed failures, and it is reported here so that a reader can see the shape of the problem and not only the parameter.
\fi


%===================================================================
\section{Cross-Lingual Retrieval}
\label{sec:cross_lingual}
%===================================================================

\label{sec:evalcorpus}
Each result is reported with the corpus state it was measured against. Grounding used \textbf{153 textbooks}, 138 English and 15 Hindi, 1{,}542 chapters and \textbf{42{,}995 passages}; two more were recovered before the retrieval ablation, which used \textbf{155 textbooks}, 1{,}557 chapters and \textbf{43{,}621 passages}. Every passage in both has an embedding, one lacking it being invisible to retrieval and not otherwise detectable. All twelve classes appear, from 3 books at class~1 to 34 at class~12.

This is 155 of the 263-book taught curriculum. The remaining 108 cannot be obtained at all. NCERT publishes no archive for most of them, one class~5 archive is corrupt, and the rest are the legacy-font Hindi readers of Section~\ref{sec:legacy}, which the pipeline rejects instead of storing as gibberish. \ifconf Each carries a recorded reason, so the shortfall can be audited. An audit traced two of the losses to this machine itself. Both have since been recovered, taking the corpus to 155 books, which is the state the retrieval ablation used; the grounding results precede the recovery and are reported against the 153-book state. \else Each is recorded with its reason, so the shortfall can be audited rather than asserted. Auditing it found two books whose loss was not the source material's. One run exhausted the disk, and one chunk embedded to a non-finite vector, which a single-transaction commit turned into a whole rolled-back book. Both have since been recovered and the corpus now holds 155 books and 43{,}621 passages, which the retrieval ablation used. The grounding results precede the recovery and have not been re-measured against the larger state. \fi

Composition matters for one of the results here. English science and mathematics books outnumber the Hindi readers roughly nine to one, which is what makes the grade-attribution result\ifconf\else\ of Section~\ref{sec:grade}\fi{} a test and not a formality. A retriever that defaults to the majority of its corpus would answer every Hindi-literature question from a science chapter.

Absolute cosine similarities should be read as characteristic of this corpus and not as a property of the method. Recall in particular is not estimated. There is no ground-truth relevant set for these books, and no such claim is made.

\ifconf\else  % journal only -- Romanized Hindi: secondary to the main ablation
\label{sec:romanized}
BGE-M3's cross-lingual capability does not extend to romanized Hindi, the predominant input mode for Indian students using standard keyboards. The same chemistry question was embedded in three scripts (Table~\ref{tab:scripts}).

\begin{table}
\caption{Script Impact on Retrieval Quality}
\label{tab:scripts}
\centering
\begin{tabular}{@{}llc@{}}
\toprule
\textbf{Script} & \textbf{Top Retrieval} & \textbf{Correct} \\
\midrule
English & Class~10 at 0.607 & Yes \\
Devanagari & Class~10 at 0.564 & Yes \\
Romanized Hindi & Class~1 at 0.520 & \textbf{No} \\
\bottomrule
\end{tabular}
\end{table}

Romanized Hindi drifts to class~1 picture books matching on surface-level token overlap. This failure provides a natural ablation study comparing naive direct retrieval (MultiRAG) against a translation-then-retrieval (tRAG) approach. Direct retrieval across scripts failed entirely. By rewriting every question into a short English search query before embedding (following Ma et al.~\cite{b4}), the vector search is successfully forced back into the correct semantic neighborhood.
\fi

\label{sec:crossling}
Retrieval was evaluated over \textbf{77 queries: seven topics in each of the eleven supported Indian languages}, against the English-medium portion of the corpus, so that language is the only variable moving within a topic. The topics span mathematics, physics, chemistry, biology and geography, were fixed before any was run, and each had to have enough English material to answer it. We commit the queries themselves alongside the benchmark, and not only their results.

How the term is built matters more than what subject it belongs to. \emph{Digestion} is ordinary vocabulary, \emph{photosynthesis} a compound of morphemes meaning ``light'' and ``joining'', \emph{Pythagoras} a transliterated proper noun; direct embedding scores 11, 2 and 5 of 11 on them, a wider spread than any difference between languages here.

Five configurations were measured (Fig.~\ref{fig:ablation}), from a lexical baseline to the deployed system. A query counts as correct only when the \emph{top} passage is on topic.


\begin{figure}
\centering
\begin{tikzpicture}
\begin{axis}[shiksha,
  width=\columnwidth, height=3.5cm,
  xbar, bar width=6pt, xmin=0, xmax=1.0,
  enlarge y limits=0.18,
  xlabel={correct at rank~1},
  xtick={0,0.25,0.5,0.75,1.0}, xticklabels={0,.25,.50,.75,1},
  symbolic y coords={BM25,{+rw},dense,{dense+rw},pooled},
  ytick=data, y dir=reverse,
  xmajorgrids, ymajorgrids=false,
]
\addplot[fill=sBlue!75, draw=sBlue!95, line width=0.5pt,
  error bars/.cd, x dir=both, x explicit,
  error bar style={black!60, line width=0.4pt}, error mark options={black!60, mark size=1.1pt}]
 table[x=val, y=cfg, x error plus=ep, x error minus=em] {
  val    cfg          ep     em
  0.000  BM25         0.048  0.000
  0.545  {+rw}        0.106  0.111
  0.610  dense        0.101  0.112
  0.857  {dense+rw}   0.061  0.095
  0.870  pooled       0.058  0.093
 };
\end{axis}
\end{tikzpicture}
\caption{The five retrieval configurations over 77 queries, with 95\% Wilson score intervals~\cite{b26}: BM25 0/77, with rewriting 42/77, dense 47/77, dense with rewriting 66/77, and the deployed pooling of both 67/77. Wilson intervals are asymmetric near the ends of the range, and are drawn as measured rather than symmetrised. The lexical baseline's interval reaches only 0.048 and excludes every other configuration; the gap between rewriting and pooling is one query and their intervals almost coincide.}
\label{fig:ablation}
\end{figure}

\ifspringer\subsection{The lexical baseline retrieves nothing at all}\else\textbf{The lexical baseline retrieves nothing at all.}\fi BM25~\cite{b25} returns zero hits for all 77 queries, not merely poor ones: a Devanagari, Tamil or Perso-Arabic query shares no tokens with an English corpus, so the inverted index has nothing to match. It is not weaker at this task but structurally incapable of it, which is the argument for carrying a 568~M-parameter embedding model on a 4~GB machine. Given an English query from the rewrite it becomes viable at once (55\%). The barrier is the script.

Multilingual RAG has been studied as a setting in its own right~\cite{b9}, including the tendency of such pipelines to answer in the language of the retrieved context rather than the question~\cite{b10}, which separating query language from answer language is designed to prevent. \ifspringer\subsection{The rewrite is where the gain is}\else\textbf{The rewrite is where the gain is.}\fi Direct embedding retrieves correctly for 47 of 77 and rewriting first for 66, a gain of nineteen queries and the largest effect measured here. It comes from normalising a term the embedding cannot decompose: on \emph{photosynthesis} direct embedding attends to the constituent morphemes, retrieves optics, and scores 2 of 11, where the rewrite recovers 10.

Pooling the two scores \textbf{67 of 77}, one query above rewriting alone, with intervals that almost coincide. The per-topic figures show where it loses: on the eye it scores 7 of 11 where direct embedding scores 9, and on the water cycle 7 against 8. Pooling merges by best score, so a confident wrong passage from one arm displaces a correct one from the other. It is deployed because it was never the worst arm on any topic. Of the ten queries it misses, nine place an on-topic passage at rank~2 or~3; one, Urdu \emph{photosynthesis}, finds nothing at all. Fig.~\ref{fig:perlang} gives it per language, as the mean reciprocal rank of that language's seven queries.

\begin{figure}
\centering
\begin{tikzpicture}[scale=1.0,font=\scriptsize]
\def\R{17.5mm}
% grid
\foreach \r in {0.25,0.5,0.75,1.0}{
  \draw[black!12, line width=0.3pt] (0,0) circle (\r*\R);
}
\draw[black!30] (0,0) circle (\R);
\foreach \i in {0,...,10}{
  \draw[black!15, line width=0.3pt] (0,0) -- (\i*32.7272+90:\R);
}
% axis labels
\foreach \i/\name in {0/Hindi,1/Urdu,2/Marathi,3/Bengali,4/Tamil,5/Telugu,
                      6/Gujarati,7/Kannada,8/Malayalam,9/Punjabi,10/Odia}{
  \node[black!70, font=\tiny] at (\i*32.7272+90:\R+3.6mm) {\name};
}
% pooled, mean reciprocal rank over the two topics
\draw[fill=sBlue!22, draw=sBlue!90, line width=0.7pt]
  (90:1.000*\R) -- (122.73:0.786*\R) -- (155.45:1.000*\R) -- (188.18:0.833*\R) --
  (220.91:1.000*\R) -- (253.64:0.857*\R) -- (286.36:0.929*\R) -- (319.09:0.857*\R) --
  (351.82:1.000*\R) -- (24.55:0.905*\R) -- (57.27:1.000*\R) -- cycle;
\foreach \a/\v in {90/1.000,122.73/0.786,155.45/1.000,188.18/0.833,220.91/1.000,
                   253.64/0.857,286.36/0.929,319.09/0.857,351.82/1.000,24.55/0.905,57.27/1.000}{
  \fill[sBlue!95] (\a:\v*\R) circle (0.7pt);
}
\end{tikzpicture}
\caption{Mean reciprocal rank of the deployed configuration for each language, averaged over that language's seven topics. Rings mark 0.25, 0.5, 0.75 and 1.0; a language reaches the outer ring when all seven of its queries put an on-topic passage first, which five of the eleven do. The shape is close to circular: the spread across languages, 0.786 to 1.000, is far narrower than the spread across topics, and the shallowest point is Urdu.}
\label{fig:perlang}
\end{figure}

\ifconf\else  % journal only -- the per-query detail behind the totals
The totals conceal which languages fail and how badly. Table~\ref{tab:perlang} gives the deployed configuration per language. Five of the eleven answer all seven topics at rank~1, and the weakest, Urdu, answers five.

\begin{table}
\caption{Pooled retrieval per language over the seven topics: queries whose top passage is on topic, and mean reciprocal rank. The eleven rank-1 counts sum to the 67 of 77 reported in Fig.~\ref{fig:ablation}.}
\label{tab:perlang}
\centering
\footnotesize
\setlength{\tabcolsep}{5pt}
\begin{tabular}{@{}lcc@{\hskip 10pt}lcc@{}}
\toprule
\textbf{Language} & \textbf{Rank-1} & \textbf{MRR} & \textbf{Language} & \textbf{Rank-1} & \textbf{MRR} \\
\midrule
Hindi     & 7/7 & 1.000 & Kannada   & 5/7 & 0.857 \\
Urdu      & 5/7 & 0.786 & Malayalam & 7/7 & 1.000 \\
Marathi   & 7/7 & 1.000 & Punjabi   & 6/7 & 0.905 \\
Bengali   & 5/7 & 0.833 & Odia      & 7/7 & 1.000 \\
Tamil     & 7/7 & 1.000 & \multicolumn{3}{l}{} \\
Telugu    & 5/7 & 0.857 & \multicolumn{3}{l}{} \\
Gujarati  & 6/7 & 0.929 & \multicolumn{3}{l}{} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Most of the shortfall is strictness, not absence.} Nine of the ten queries that fail the rank-1 criterion place an on-topic passage at rank~2 or~3. Scoring a hit anywhere in the top five would report 76 of 77; a student reads what is first, and the strict criterion is what leaves the one real failure visible.

\textbf{One query finds nothing at all.} Urdu \emph{photosynthesis} is the single case in 77 where no on-topic passage appears anywhere in the top five. The Urdu term \emph{ziyai talif} is Perso-Arabic in origin and shares no root with either the English word or the Sanskrit-derived compounds the other ten languages use, so neither arm has a handle on it. The Tamil photosynthesis query, which failed the same way in the earlier 22-query run, now succeeds: the rewrite normalised the term instead of paraphrasing it away. That is the non-determinism of the rewrite showing up as a change in which query fails, not as a change in how many.
\fi


\ifspringer\subsection{What 77 queries can and cannot establish}\else\textbf{What 77 queries can and cannot establish.}\fi Two conclusions are secure: the lexical baseline's interval reaches only 0.048 and excludes every other configuration, so its failure is structural; and rewriting gains nineteen queries over direct embedding, which no reading of the sample size removes. One is not. Pooling scores a single query above rewriting, its interval sits inside rewriting's, and on two topics it is worse than the better arm; it is reported as what the system deploys.

\ifspringer\else
The rewrite is a language-model call and is not deterministic at temperature zero. Repeated runs at 22 queries returned 16 and 17 with the ordering stable; the 77-query figures come from a single run and carry the same instability, so none is read more tightly than its interval allows. The same non-determinism produced the earlier failure in which a mistranslated rewrite (``Rahim'' as ``Rahman'') silently replaced a question that would have retrieved correctly.
\fi

\ifshort\else One query fails in every configuration: Urdu \emph{photosynthesis}. The term (\textit{ziyai talif}, Perso-Arabic in origin) shares no morphology with the English word or with the Sanskrit-derived compounds the other ten languages use, so neither arm has anything to match.\fi

%===================================================================
\section{Memory Optimization}
%===================================================================
\label{sec:memory}

\label{sec:precision}
BGE-M3's weights were evaluated at both full (float32) and half (float16) precision (Table~\ref{tab:precision}).

\begin{table}
\caption{Precision Comparison for BGE-M3}
\label{tab:precision}
\centering
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Metric} & \textbf{float32} & \textbf{float16} \\
\midrule
Weight memory & 2,166~MB & 1,083~MB \\
Encode 4 queries & 2,255~ms & 1,197~ms \\
Model load time & 8.5~s & 8.1~s \\
Cosine fidelity (mean) & --- & 0.999998 \\
Cosine fidelity (min) & --- & 0.999964 \\
\bottomrule
\end{tabular}
\end{table}

Half precision halves the weights and encodes ${\sim}1.88\times$ faster. Fidelity, measured over 400 corpus passages, is 0.999998 on average and 0.999964 at worst. Whether that matters is a question about rankings, so it was tested as one: over eight queries the top twelve passages are the same twelve under both precisions, in the same order for six of the eight, with the first difference at rank nine. Context is assembled from six passages, so nothing reported here is affected. Half precision can still reorder a list, and we report that it does.


\ifspringer\else
\textbf{Measurement against arithmetic.} An earlier analysis summed component sizes to ${\sim}1.6$~GB and concluded comfortable fit within 4~GB. A dedicated benchmarking script measures actual peak RSS through \texttt{getrusage(RUSAGE\_SELF)}, a high-water mark surviving page eviction, unlike \texttt{ps} which reported 46~MB for a process holding a 1~GB model \ifconf\else\ (Table~\ref{tab:footprint})\fi.
\fi

\ifconf\else  % journal only -- every figure in it is stated in the prose below
\begin{table}
\caption{Serving Pipeline Memory Footprint (Measured)}
\label{tab:footprint}
\centering
\begin{tabular}{@{}lr@{}}
\toprule
\textbf{Pipeline Stage} & \textbf{Peak RSS} \\
\midrule
Interpreter start & 14~MB \\
Configuration imported & 265~MB \\
BGE-M3 loaded (fp16) & 534~MB \\
\textbf{Query encoded} & \textbf{2,094~MB} \\
pgvector search complete & 2,094~MB \\
\bottomrule
\end{tabular}
\end{table}
\fi

The actual peak is \textbf{2,094~MB}. The prediction was 1,600~MB\ifconf\else\ (Fig.~\ref{fig:memory})\fi. Loading model weights accounts for only 534~MB (\texttt{safetensors} memory-maps them). The first forward pass adds 1,560~MB as weight pages become resident and attention workspaces are allocated. The system fits within 4~GB with ${\sim}1.4$~GB usable headroom (accounting for ${\sim}500$~MB of operating system overhead), where the arithmetic had implied 2.4~GB.

\ifconf\else  % journal only: every number in it is in the prose alongside
\begin{figure}
\centering
\begin{tikzpicture}
\begin{axis}[shikshabar,
  width=0.94\columnwidth, height=3.8cm,
  ymin=0, ymax=2450, ylabel={peak resident set (MB)},
  symbolic x coords={start,config,loaded,encoded,searched},
  xtick=data, x tick label style={font=\tiny},
  enlarge x limits=0.15,
  extra y ticks={1600}, extra y tick labels={}, extra y tick style={grid=major, grid style={black!45, dashed}},
]
\addplot coordinates {(start,14) (config,265) (loaded,534) (encoded,2094) (searched,2094)};
\end{axis}
\end{tikzpicture}
\caption{Peak resident set through the serving pipeline, from interpreter start through configuration import, model load, the first query encoding, and a completed vector search. The dashed line is the 1,600~MB an earlier analysis predicted by summing component sizes. Loading the weights accounts for 534~MB because \texttt{safetensors} memory-maps them; the 1,560~MB that follows arrives on the first forward pass, when those pages become resident and the attention workspaces are allocated.}
\label{fig:memory}
\end{figure}
\fi

\textbf{Why the reranker is absent.} \ifshort A cross-encoder is the usual answer to imperfect ranking, so its absence needs more than a footprint estimate. Over the same 77 queries and pooled candidates, BGE-Reranker-v2-M3 scores \textbf{65 of 77} against 67 without it, promoting seven and demoting nine; resident beside the embedder it drove an 8~GB machine to 12.5~GB of swap without finishing. It is excluded on that evidence.\else A cross-encoder scores question and passage jointly and is the usual answer to imperfect ranking, so its absence from the retrieval path needs more than a footprint estimate. Over the same 77 queries and the same pooled candidates, BGE-Reranker-v2-M3 scores \textbf{65 of 77} against 67 without it, promoting seven queries to rank~1 and demoting nine, with intervals that overlap almost entirely. It also does not fit: held resident beside the embedder the pair requested 8.6~GB on an 8~GB machine, drove swap to 12.5~GB and 435{,}654 page-ins, and had not finished after fifty-one minutes.\ifconf\else\ Releasing the embedder first, which a deployment cannot do because it needs it to search, the run still spent thirty-two minutes of wall time to accumulate four minutes of CPU. The figures quoted are paging counters rather than resident set, which stops being meaningful once pages are being evicted.\fi{} It is excluded on measurement, not assumption.\fi

\ifconf\else  % journal only -- Ingestion memory: an operations note
A single process ingesting 263 books reached book~8 and entered an unrecoverable state: swap at 11.4~GB of 12.2~GB maximum, the process in uninterruptible I/O wait. \texttt{SIGKILL} is not delivered in that state. The solution is batch processing: one process per six books, with ${\sim}20$~seconds of model reloading per batch.
\fi

\ifconf\else  % journal only -- accessibility: a separate contribution, not the retrieval story
%===================================================================
\section{Reading Support for Brahmic Script Readers}
\label{sec:accessibility}
%===================================================================

Dyslexia support tooling is overwhelmingly designed for English. Most Indian students read a Brahmic script where the fundamental unit is the \textbf{akshara}, a consonant cluster carrying an inherent or explicit vowel. The segmentation algorithm recognizes the virama (halant) in each Brahmic script as the signal that two consonants are conjoined, operating generically across Devanagari, Bengali, Gurmukhi, Gujarati, Odia, Tamil, Telugu, Kannada, and Malayalam.

Two details: (1)~sentence counting must honor the danda, not only the Latin period; (2)~Flesch-Kincaid is computed for English only, since akshara counts are not syllable counts.

The first simplification attempt produced text \textit{harder} than the original (Table~\ref{tab:readability}).

\begin{table}
\caption{Readability Measurement: Flesch-Kincaid Grade Level}
\label{tab:readability}
\centering
\begin{tabular}{@{}lr@{}}
\toprule
\textbf{Version} & \textbf{FK Grade} \\
\midrule
NCERT source text & 7.0 \\
First simplification attempt & \textbf{8.2} (harder) \\
Measure-and-keep approach & \textbf{6.9} (easier) \\
\bottomrule
\end{tabular}
\end{table}

The corrected approach measures the original, rewrites with readability as sole objective, and retains whichever version is easier.

Letter and word spacing (on by default) is the only intervention with strong evidence~\cite{b13}. Dyslexia-specific fonts are offered as preferences but labeled as weakly evidenced~\cite{b14,b15}. Nothing in the module diagnoses any condition. It adjusts presentation.
\fi

\ifconf\else  % journal only -- Security audit: an engineering result, not a retrieval one
%===================================================================
\section{Security Analysis}
%===================================================================

A comprehensive audit identified twelve vulnerabilities, all repaired (Table~\ref{tab:security}).

\begin{table}
\caption{Security Audit Findings (Selected)}
\label{tab:security}
\centering
\begin{tabular}{@{}cp{5.5cm}@{}}
\toprule
\textbf{\#} & \textbf{Vulnerability / Consequence} \\
\midrule
1 & Safety pipeline import error caught by bare \texttt{except}: every response marked safe without evaluation \\
2 & JWT \texttt{type} claim unchecked: 7-day refresh tokens accepted as 30-minute access tokens \\
3 & \texttt{validate\_required()} never called: production booted with no JWT secret \\
5 & Embedding column as \texttt{double precision[]}: no vector index possible \\
7 & RLS compared \texttt{NULL=NULL}: shared curriculum invisible to all users \\
9 & Sentry received student PII despite \texttt{send\_default\_pii=False} \\
\bottomrule
\end{tabular}
\end{table}

Vulnerability \#1 is the most consequential. The safety pipeline was designed as a three-pass content filter, but an import path error caused the entire pipeline to fail to load. The error was caught by a bare \texttt{except} clause returning \texttt{(response, True)}, making the content safety system structurally incapable of blocking any response since deployment.

\textbf{A policy that hid the curriculum from everybody (\#7).} Multi-tenancy placed row-level security on the content table with a single policy:

\begin{center}\footnotesize
\texttt{USING (organization\_id =}\\
\texttt{~~current\_setting('app.current\_org\_id')::uuid)}
\end{center}

That is correct for a school's own uploads and wrong for NCERT textbooks, which belong to no school and must be readable by every student. A shared row has \texttt{organization\_id IS NULL}, so the policy evaluates \texttt{NULL = NULL}, which is \texttt{NULL}, which PostgreSQL treats as false. Shared content was therefore not merely unowned but invisible to every user, permanently, and ingesting the curriculum at all was impossible. The repair adds a second policy under which a \texttt{NULL} organization means shared. Policies for the same command are OR'd, so tenant isolation is untouched and a row owned by one school remains visible only to that school. Writes were deliberately not opened. The existing policy has no \texttt{WITH CHECK}, so PostgreSQL applies its \texttt{USING} clause as the insert check and a tenant still cannot promote its own content to global. Only the table owner, which is what migrations and the ingestion job connect as, can publish shared curriculum.

This defect is characteristic of the class the audit kept finding. It is not a missing check but a check that is present, plausible, and evaluates to the wrong thing on a value nobody tested with. It produced no error, no log line and no failed request. It produced an empty result set, which is indistinguishable from a corpus that has not been ingested yet.

\textbf{A token type nobody checked (\#2).} The JWT implementation validated signature and expiry but not the \texttt{type} claim, so a refresh token, issued with a seven-day lifetime for the sole purpose of obtaining new access tokens, was accepted anywhere an access token was expected. The practical effect is that the thirty-minute access-token lifetime, and every security argument resting on it, was fourteen times longer than stated for any client holding a refresh token. The repair checks the claim and the tests now assert the rejection, because a test that only exercises the valid path cannot fail when a validation step is removed.

\fi
%===================================================================
\section{Grounding and Drift Detection}
%===================================================================
\label{sec:grounding}

A retrieval-augmented system can retrieve the correct chapter and still generate a fabricated answer, which is the failure mode surveyed across generation tasks by Ji et al.~\cite{b29} and the one retrieval is supposed to remove. When asked about a specific story from the Hindi curriculum, the system retrieved the correct chapter, then described events that never occurred. The class was correct, the chapter was correct, the language was correct, and every detail was invented.

The \textbf{grounding score} is the cosine similarity of the answer to its source passages. Reference-free RAG evaluation frameworks score a comparable property, faithfulness, with a language model as judge~\cite{b27}; the metric used here is a single cosine similarity, which costs one encoder pass and no second model. Lexical overlap cannot serve the purpose because the platform produces answers in a different language from the source passages. BGE-M3's cross-lingual embedding property makes the metric functional in exactly this case.

\ifspringer\subsection{Calibrating on a distribution}\else\textbf{Calibrating on a distribution.}\fi The threshold was originally set at 0.55 from four hand-verified answers, three correct at 0.583--0.701 and one fabricated at 0.489. Four points that happen to separate are not a calibration, and the difficulty is obtaining ungrounded answers in quantity. A model instructed to hallucinate does not fail the way a grounded pipeline fails.

We therefore construct the negatives. Every answer is generated normally from its own retrieved passages, then scored twice: against the passages it was written from (\emph{matched}), and against another question's passages (\emph{mismatched}). A mismatched pair is precisely the failure the metric exists to catch (fluent, well-formed, on a different subject) and it requires no human labelling and no second generation pass. Sixteen questions spanning subjects and classes yield 16 matched and 240 mismatched pairs.

Grounding need not be measured against the concatenation of the retrieved passages, so four estimators were compared over identical generations (Fig.~\ref{fig:estimators}). The two runs of the experiment, with independently generated answers, agreed to within 0.007 on every entry.


\begin{figure}
\centering
\begin{tikzpicture}
\begin{axis}[shiksha,
  width=\columnwidth, height=4.1cm,
  ybar, bar width=6pt, ymin=0, ymax=1.0,
  ylabel={cosine similarity},
  symbolic x coords={concat,{best psg.},{top 3},sentence},
  xtick=data, enlarge x limits=0.18,
  legend style={at={(0.5,1.14)}, anchor=north, legend columns=2},
]
\addplot[fill=sBlue!80, draw=sBlue!95, line width=0.5pt,
  error bars/.cd, y dir=both, y explicit]
 coordinates {(concat,0.830) +- (0,0.044) ({best psg.},0.854) +- (0,0.047)
              ({top 3},0.822) +- (0,0.053) (sentence,0.676) +- (0,0.047)};
\addplot[fill=sOrange!75, draw=sOrange!95, line width=0.5pt,
  error bars/.cd, y dir=both, y explicit]
 coordinates {(concat,0.470) +- (0,0.069) ({best psg.},0.502) +- (0,0.068)
              ({top 3},0.486) +- (0,0.067) (sentence,0.455) +- (0,0.049)};
\legend{matched, mismatched}
\end{axis}
\end{tikzpicture}
\caption{Four grounding estimators over 16 matched and 240 mismatched pairs, with one standard deviation. What separates the populations is the distance between the two bars. Their heights do not matter.}
\label{fig:estimators}
\end{figure}

The simplest estimator is also the best. Scoring the answer against all retrieved passages joined separates the two populations by 0.360, and no refinement improves on it. Scoring against the single best passage raises both means without widening the gap, and scoring sentence by sentence narrows it, because a correct answer contains connective and pedagogical sentences that appear in no passage, and averaging over them dilutes the signal the metric depends on.

\ifspringer\subsection{The boundary was in the wrong place}\else\textbf{The boundary was in the wrong place.}\fi The distribution shows that 0.55 separates the populations but sits too low. Thirty-five of the 240 mismatched pairs, one drifted answer in seven, score above it and pass as grounded. At 0.715 none do, while no matched answer falls below it. Balanced accuracy rises from 0.927 to 1.000, and the number of correct answers needlessly regenerated stays at zero. The deployed threshold was raised accordingly. The original four points were correct. There were simply too few of them to locate the boundary.

That 1.000 should be read for what it measures. The negatives are answers written from \emph{another question's} passages, so the task the score is perfect at is separating an answer from a different subject: topical drift, which is the failure we observed in deployment. It is not a measurement of hallucination detection in general, and an answer that stays on its retrieved topic while inventing a detail inside it would score as grounded here. The threshold is calibrated for the failure it was built to catch, and we claim no more than that.

\label{sec:grounding-indep}
\textbf{The embedder is not judging its own geometry.} BGE-M3 retrieved these passages and then scored the answers against them, so the separation could be a property of one embedding space and not of the text. Re-scoring the same generations settles it. Character 3--5 gram TF-IDF, which uses no neural model at all, separates the populations by 0.359, and a distilled multilingual MiniLM by 0.471, against BGE-M3's 0.360 (balanced accuracy 1.000, 0.996, 1.000). The gap is a property of the answers. Only the threshold is particular to the embedder deployed.

\ifconf\else  % journal only -- Consolidated Results: every figure in it is stated where it was measured
%===================================================================
\section{Consolidated Results}
%===================================================================

The platform is validated by \textbf{490 automated tests}, of which \textbf{457 pass and 33 are skipped, with zero failures and zero errors.} The skips are declared, not silent. They are tests requiring a loaded model or a service this environment does not run, and the counts here are read from the suite's own JUnit report, not from a summary line.

Table~\ref{tab:results} summarizes key metrics.

\begin{table}
\caption{Consolidated System Metrics}
\label{tab:results}
\centering
\begin{tabular}{@{}lr@{}}
\toprule
\textbf{Metric} & \textbf{Value} \\
\midrule
NCERT catalog coverage & 559 textbooks \\
Taught curriculum & 263 textbooks \\
Ingested corpus (grounding / retrieval) & 153 / 155 textbooks \\
Supported languages & 11 \\
API routes & 72 \\
Ingestion throughput & 140 s/book (1.38$\times$) \\
Ingestion time, 153 books & ${\sim}6.0$ hours \\
Peak serving memory (fp16) & 2,094~MB \\
fp16 cosine fidelity (mean) & 0.999998 \\
Memory reduction (fp16) & 50\% \\
Encoding speedup (fp16) & 1.88$\times$ \\
Response latency (warm) & ${\sim}3$~s \\
Grounding threshold (calibrated) & 0.715 \\
Grounding balanced accuracy & 1.000 \\
Security vulns fixed & 12 \\
Automated tests & 457 pass, 33 skip, 0 fail \\
\bottomrule
\end{tabular}
\end{table}

\fi
%===================================================================
\ifconf\else  % journal only -- the extended account
%===================================================================
%===================================================================
\section{The Rewrite as a Failure Surface}
\label{sec:rewrite}
%===================================================================

The query rewrite is the one component that improves retrieval and degrades it in the same breath, and the ablation of Section~\ref{sec:crossling} exists because reasoning alone could not say which effect dominated.

Rewriting normalises a compound term to its English canonical form, which is why it lifts photosynthesis retrieval from 2 of 11 to 10 of 11. It also paraphrases, and a paraphrase can be less precise than the original. Three ordinary-vocabulary queries that direct embedding answers correctly are lost when rewritten. The two arms fail on disjoint inputs, so neither dominates and the union outperforms both. This is a property of these failure modes, not a general claim about rewriting.

Three consequences follow from the rewrite being a generation and not a transformation, and all three are visible in the measurements.

It is \emph{not deterministic} at temperature zero. Repeated runs at 22 queries returned 16 and 17, and between that run and the 77-query run reported here the Tamil photosynthesis query changed from failing to succeeding, which is why no retrieval figure in this paper is given more precision than its interval carries.

It can \emph{silently substitute}. An early failure replaced ``Rahim'' with ``Rahman'' in a rewritten question about a Hindi poet, and the pipeline then retrieved correctly for a question the student had not asked. Nothing in the retrieval path can detect this. The retrieval succeeded, the grounding score was high, and the answer was coherent. The defect is upstream of every check the system has.

It is \emph{hosted}. The rewrite leaves the machine, so a system whose corpus and index are local still transmits the question. The paper states this in the abstract because the alternative, describing the platform as offline on the strength of local retrieval, is the claim this project has already had to retract once.

Pooling both retrievals is not an ensemble in any interesting sense. It is a union of two candidate sets before ranking, costing one extra vector search of approximately 20~ms. Choosing between the arms per query would require knowing in advance whether the query contains a compound term, which is the classification problem the rewrite was introduced to avoid. The union is the configuration that requires no such knowledge, and the measurement is what justifies preferring it.

%===================================================================
\section{Deployment and Portability}
\label{sec:deployment}
%===================================================================

The 4~GB target is a claim about the machines Indian schools own, and those are predominantly Windows desktops. A memory measurement taken on Apple hardware is evidence about the software's footprint and not about performance on the target, and the gap between the two is where portability defects live.

The dependency manifest carried no environment markers, so an installation on any platform fetched \texttt{mlx}, \texttt{mlx-lm} and \texttt{coremltools}. None of the three publishes a wheel outside macOS and the first two are Apple Silicon only, so installation on Windows aborted before any other dependency was fetched. The code was already prepared for their absence. Every import of them sits inside a function behind \texttt{try}/\texttt{except}, and device selection already falls back through CUDA to CPU, so the platform the paper targets was excluded by the manifest alone. The markers now confine them to Apple Silicon, and an unused \texttt{python-magic} dependency was removed instead of marked. Nothing imports it, and on Windows it installs cleanly and then fails at runtime looking for a library pip does not supply.

Everything the platform needs to know about its host (installed memory, available memory, swap, peak resident set) is read through a single abstraction over \texttt{psutil}, which reports identically on Linux, Windows and macOS. That abstraction replaced direct system calls answering only on macOS: \texttt{sysctl hw.memsize}, \texttt{memory\_pressure}, \texttt{sysctl vm.swapusage} and \texttt{getrusage}. One call site sat outside an exception handler and therefore raised instead of degrading, and the benchmark substantiating the 4~GB figure imported the POSIX-only \texttt{resource} module at module scope, making it unrunnable on precisely the platform whose claim it exists to support.

Peak resident set is the one quantity with no portable source. \texttt{getrusage} is POSIX-only. Windows exposes a peak through the process handle. The abstraction reads whichever exists and returns a flag when it has fallen back to current resident set, rather than presenting a smaller number as a high-water mark. A related unit defect is worth recording because it is silent: \texttt{ru\_maxrss} is bytes on macOS and kilobytes on Linux, so identical code reports figures a factor of 1024 apart on two POSIX systems.

We have removed the known obstacles to installing and running on Windows and Linux, and we have not measured the result on either. No figure in this paper is offered for a platform we did not run on, and the portability work is reported as the removal of defects, not as evidence of performance.


\section{Failures That Reported Success}
\label{sec:failures}
%===================================================================

Three defects in this project shared a property worth naming. Each produced a system that reported success while doing nothing. They are recorded because the measurement discipline in this paper exists in response to them, and because a reader deciding how much to trust the numbers is entitled to know what the numbers replaced.

Vector search had never run. The retrieval code queried with the pgvector cosine operator while \texttt{embeddings.embedding} was declared \texttt{double precision[]}, so every search failed on \texttt{operator does not exist: double precision[] <=> vector}. The migration that should have created the HNSW indexes instead skipped them, logging \emph{``bypassing HNSW index creation due to missing pgvector extension''}, created a plain btree named \texttt{idx\_embeddings\_content\_id\_hnsw}, and then printed that it had built three HNSW indexes. The index name and the log message both described an object that did not exist.

Underneath was an environment fault, not a coding one. Homebrew builds pgvector only for the PostgreSQL versions it packages, so on the 14.19 server in use the extension was present for 17 and 18 and absent for 14. \texttt{CREATE EXTENSION} had been attempted, had failed, and the failure had been handled by continuing.

A book commits in a single transaction, so a failure rolls it back entirely and the book never appears in the database. The batched runner selects whatever is \emph{not} in the database. Any book that fails deterministically is therefore selected again on every pass, forever. One overnight run spent \textbf{1,178 batches to ingest 16 books}: a class~5 archive with a \texttt{Bad CRC-32}, and legacy-font Hindi readers whose chapters are all correctly rejected, were chosen and re-chosen indefinitely. Books returning HTTP~404 already left a marker and were skipped. These two failure modes left none. The fix is the dead-end record of Section~\ref{sec:deadends}, and its own limitation, that it does not distinguish a transient cause from a permanent one, is the defect reported there.

The application role held privileges on none of the 42 tables. Reads returned empty instead of raising, so an empty corpus and an unreadable corpus were indistinguishable from the application's point of view, and the ingestion that appeared to succeed had written nothing the serving path could see.

%===================================================================
\section{Threats to Validity}
\label{sec:threats}
%===================================================================

\textbf{Construct.} The grounding evaluation scores an answer against passages. A fluent answer reusing the vocabulary of its context scores well whether or not its claims are true. Section~\ref{sec:grounding-indep} shows the separation is not an artefact of one embedder, which is a different question from whether cosine similarity measures factual grounding. It does not. It measures whether the answer is about the passages it was given.

\textbf{Internal.} Every retrieval and grounding number comes from one corpus state, one machine, and one hosted generation endpoint. The rewrite and the generation are model calls that are not deterministic at temperature zero, which is why the pooled retrieval figure is reported as $16\pm1$ and the grounding experiment was run twice with independently generated answers before either was reported.

\textbf{External.} The evaluation covers seven topics per language and one curriculum. Nothing here establishes that the ordering of retrieval configurations transfers to another corpus, and the absolute cosine similarities certainly do not. The 4~GB claim is a measurement of peak resident set on one Apple~M1. It is evidence about the software's footprint, not about performance on the Windows machines the target deployment assumes.

\textbf{Absent.} There is no human evaluation. No teacher or student has assessed whether an answer this system produces is pedagogically useful, and no measurement in this paper substitutes for that. A system that retrieves the right chapter, stays grounded in it, and fits in memory may still teach badly.

%===================================================================
\section{Reproducibility}
\label{sec:repro}
%===================================================================

Each result in this paper is produced by a committed script, named in Table~\ref{tab:repro}, run against a recorded corpus state. The corpus is 153 textbooks, 1{,}542 chapters and 42{,}995 passages with an embedding for every passage. The catalog, the ingestion pipeline and the dead-end markers reconstruct it from NCERT's published archives without redistributing them. The cross-lingual queries are committed in their original scripts, not transliterated, so a reader can inspect what was actually asked.

\begin{table}
\caption{What produces each reported result.}
\label{tab:repro}
\centering
\footnotesize
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{Result} & \textbf{Script} \\
\midrule
Cross-lingual ablation & \texttt{cross\_lingual\_retrieval.py} \\
Grounding calibration  & \texttt{grounding\_estimators.py} \\
Estimator independence & \texttt{grounding\_independent.py} \\
Encoder batch check    & \texttt{grounding\_batch\_check.py} \\
Serving footprint      & \texttt{serving\_footprint.py} \\
\bottomrule
\end{tabular}
\end{table}

The grounding scripts cache their generations, so a re-run re-scores identical answers and separates a change in the metric from a change in what the model said. That distinction was not academic. An early run of the calibration reported balanced accuracy 0.451, and because it cached nothing its generations cannot be examined and it cannot be explained. Every figure reported here comes from a run whose inputs survive.
\fi


\section{Discussion}
%===================================================================

\ifconf
One pattern recurs throughout this work. Measurement kept contradicting what we had assumed, and we learned more from the contradictions than from the assumptions.
\else
The measurement-driven methodology reveals a recurring pattern. Intuitive engineering assumptions are frequently contradicted by empirical measurement, and the contradictions are more informative than the assumptions.
\fi

\ifconf\else  % journal only -- the worked example; the pattern is stated above
The legacy font detection illustrates this most starkly. A quality metric searching for damaged Devanagari ranked the most corrupted books as the best, because corruption manifested as the \textit{absence} of Devanagari, which the metric interpreted as perfection. The memory estimation follows the same pattern. Summing static sizes to 1.6~GB misses the 2.1~GB transient peak. The grounding score addresses a fundamental RAG limitation. A system can retrieve correctly, generate fluently, pass every surface check, and still fabricate content.
\fi

\textbf{Availability.} The platform, the ingestion pipeline and every benchmark reported here, including the 77 cross-lingual queries in their original scripts and the grounding calibration, are available at \url{https://github.com/rachitranka25/SHIKSHASETU}. The corpus itself is not redistributed. The pipeline reconstructs it from NCERT's own published archives.

\textbf{Rights and ethics.} NCERT publishes these textbooks for free public download. The system stores only locally derived text and vectors, redistributed with neither the code nor this paper, and a deployment beyond research use should confirm the applicable terms; availability alone does not settle them. No human subjects took part and no student data was collected. Every measurement here is taken from the system itself.

\textbf{Limitations.} (1)~We construct the grounding negatives by mismatching answers to other questions' passages, so they are not drawn from observed hallucinations. This captures topical drift, which is the failure we have seen, but not an answer that stays on topic and invents a detail within it. (2)~Cross-lingual retrieval is evaluated on two topics per language. Broader topic coverage would tighten the estimate. (3)~27 legacy-font Hindi books remain unprocessable. (4)~Memory measurements have run-to-run variance of up to 414~MB.

%===================================================================
\section{Conclusion and Future Work}
%===================================================================

Shiksha Setu is a local-retrieval AI tutoring platform whose pipeline addresses the complete NCERT catalog of 559 textbooks and which delivers instruction in eleven Indian languages on consumer hardware with as little as 4~GB of RAM. The reported evaluation rests on 155 ingested textbooks and 43{,}621 indexed passages spanning all twelve classes, with grounding measured against the 153-book state that preceded them. Every performance claim has been substantiated through kernel-level measurement, and engineering failures have been documented alongside their corrections.

\ifconf
The principal contributions are: (1)~corpus construction with legacy-font corruption detection affecting 66\% of Hindi textbooks, (2)~cross-lingual retrieval measured over 77 queries in eleven languages, with an ablation showing that rewriting the query before embedding it gains nineteen queries over embedding it directly, 66 of 77 against 47, and that a lexical baseline retrieves none of the 77, (3)~a grounding metric separating grounded from drifted answers by 0.360 in cosine similarity over 256 pairs, at a calibrated threshold that admits none of the 240 negatives where the previously shipped value admitted 35, and (4)~half-precision inference halving memory at a mean cosine fidelity of 0.999998. Reading support for Brahmic scripts and a security audit of the deployment are reported in the extended version.
\else
The principal contributions are: (1)~corpus construction with legacy-font corruption detection affecting 66\% of Hindi textbooks, (2)~cross-lingual retrieval measured over 77 queries in eleven languages, with an ablation showing that rewriting the query before embedding it gains nineteen queries over embedding it directly, 66 of 77 against 47, and that a lexical baseline retrieves none of the 77, (3)~a grounding metric separating grounded from drifted answers by 0.360 in cosine similarity over 256 pairs, at a calibrated threshold that admits none of the 240 negatives where the previously shipped value admitted 35, (4)~half-precision inference halving memory at a mean cosine fidelity of 0.999998, (5)~akshara-level readability for nine Brahmic scripts, and (6)~a security audit repairing twelve production vulnerabilities.
\fi

\ifconf
Future work targets a human-in-the-loop pilot with practising teachers, negatives drawn from observed hallucinations, a cross-encoder reranker for the compound-term failures, and a Walkman-Chanakya to Unicode transliterator to recover the legacy-font books.
\else
Future work targets: conducting a human-in-the-loop pilot study with actual educators to evaluate pedagogical utility, expanding the hallucination annotation dataset (100+ response pairs) for robust grounding calibration, integrating a cross-encoder reranker to resolve compound-term failures (e.g., photosynthesis), implementing Entity Linking for polysemous term disambiguation, and developing a Walkman-Chanakya-to-Unicode transliterator.
\fi

% The conference version is held to a page limit and the reference list is
% the last thing that should be cut to meet it, so it is set smaller instead.
% The reference list runs a few lines past the page. Tightening the space
% between entries recovers them without dropping a citation or shrinking
% the type, both of which cost the reader more than the leading does.
\begingroup
\setlength{\itemsep}{0pt plus 0.2pt}
\begin{thebibliography}{00}
\setlength{\itemsep}{0pt plus 0.2pt}\setlength{\parskip}{0pt}
% Conference page limit: the reference list is the last thing that should be cut to
% meet it, so it is set smaller and tighter instead of shorter.
\ifspringer\else\ifshort\scriptsize\else\ifconf\footnotesize\fi\fi\fi
\ifconf
  \setlength{\itemsep}{0pt}%
  \setlength{\parskip}{0pt}%
  % A narrower hanging indent gives each entry more line width, so the
  % list loses lines without losing references.
  \setlength{\leftmargin}{11pt}%
  \setlength{\labelwidth}{9pt}%
  \setlength{\labelsep}{2pt}%
  \setlength{\itemindent}{0pt}%
\fi
\ifspringer\bibitem{b1} Kakwani, D., et al.: IndicNLPSuite: corpora, benchmarks and pre-trained models for Indian languages. In: Findings of EMNLP (2020)
\else\bibitem{b1} D. Kakwani \textit{et al.}, ``IndicNLPSuite: corpora, benchmarks and pre-trained models for Indian languages,'' in \textit{Findings of EMNLP}, 2020.\fi
\ifspringer\bibitem{b2} Lewis, P., et al.: Retrieval-augmented generation for knowledge-intensive NLP tasks. In: Proc. NeurIPS (2020)
\else\bibitem{b2} P. Lewis \textit{et al.}, ``Retrieval-augmented generation for knowledge-intensive NLP tasks,'' in \textit{Proc. NeurIPS}, 2020.\fi
\ifspringer\bibitem{b3} Karpukhin, V., et al.: Dense passage retrieval for open-domain question answering. In: Proc. EMNLP, pp. 6769--6781 (2020)
\else\bibitem{b3} V. Karpukhin \textit{et al.}, ``Dense passage retrieval for open-domain question answering,'' in \textit{Proc. EMNLP}, 2020, pp. 6769--6781.\fi
\ifspringer\bibitem{b4} Ma, X., Gong, Y., He, P., Zhao, H., Duan, N.: Query rewriting for retrieval-augmented large language models. In: Proc. EMNLP (2023)
\else\bibitem{b4} X. Ma, Y. Gong, P. He, H. Zhao, and N. Duan, ``Query rewriting for retrieval-augmented large language models,'' in \textit{Proc. EMNLP}, 2023.\fi
\ifspringer\bibitem{b5} Nogueira, R., Cho, K.: Passage re-ranking with BERT. arXiv:1901.04085 (2019)
\else\bibitem{b5} R. Nogueira and K. Cho, ``Passage re-ranking with BERT,'' \textit{arXiv preprint arXiv:1901.04085}, 2019.\fi
\ifspringer\bibitem{b6} Chen, J., Xiao, S., Zhang, P., Luo, K., Lian, D., Liu, Z.: BGE M3-Embedding: multi-lingual, multi-granularity text embeddings via self-knowledge distillation. arXiv:2402.03216 (2024)
\else\bibitem{b6} J. Chen, S. Xiao, P. Zhang, K. Luo, D. Lian, and Z. Liu, ``BGE M3-Embedding: multi-lingual, multi-granularity text embeddings via self-knowledge distillation,'' \textit{arXiv preprint arXiv:2402.03216}, 2024.\fi
\ifspringer\bibitem{b7} Gala, A., et al.: IndicTrans2: accessible machine translation for all 22 scheduled Indian languages. Trans. Mach. Learn. Res. (TMLR) (2023)
\else\bibitem{b7} A. Gala \textit{et al.}, ``IndicTrans2: accessible machine translation for all 22 scheduled Indian languages,'' \textit{Trans. Mach. Learn. Res. (TMLR)}, 2023.\fi
\ifspringer\bibitem{b8} Khanuja, S., et al.: MuRIL: multilingual representations for Indian languages (2021)
\else\bibitem{b8} S. Khanuja \textit{et al.}, ``MuRIL: Multilingual representations for Indian languages,'' 2021.\fi
\ifspringer\bibitem{b9} Xu, Z., et al.: Multilingual retrieval-augmented generation for knowledge-intensive task. arXiv:2504.03616 (2025)
\else\bibitem{b9} Z. Xu \textit{et al.}, ``Multilingual retrieval-augmented generation for knowledge-intensive task,'' \textit{arXiv preprint arXiv:2504.03616}, 2025.\fi
\ifspringer\bibitem{b10} Chen, Y., et al.: Language drift in multilingual RAG: characterization and decoding-time mitigation. arXiv:2511.09984 (2025)
\else\bibitem{b10} Y. Chen \textit{et al.}, ``Language drift in multilingual RAG: characterization and decoding-time mitigation,'' \textit{arXiv preprint arXiv:2511.09984}, 2025.\fi
\ifspringer\bibitem{b11} Malkov, Y.A., Yashunin, D.A.: Efficient and robust approximate nearest neighbor search using HNSW graphs. IEEE Trans. Pattern Anal. Mach. Intell. \textbf{42}(4), 824--836 (2020)
\else\bibitem{b11} Y. A. Malkov and D. A. Yashunin, ``Efficient and robust approximate nearest neighbor search using HNSW graphs,'' \textit{IEEE Trans. Pattern Anal. Mach. Intell.}, vol. 42, no. 4, pp. 824--836, Apr. 2020.\fi
\ifspringer\bibitem{b12} Reimers, N., Gurevych, I.: Sentence-BERT: sentence embeddings using Siamese BERT-Networks. In: Proc. EMNLP, pp. 3982--3992 (2019)
\else\bibitem{b12} N. Reimers and I. Gurevych, ``Sentence-BERT: Sentence embeddings using Siamese BERT-Networks,'' in \textit{Proc. EMNLP}, 2019, pp. 3982--3992.\fi
\ifconf\else  % journal only -- cited only by the accessibility and readability sections
\ifspringer\bibitem{b13} Zorzi, M., et al.: Extra-large letter spacing improves reading in dyslexia. Proc. Nat. Acad. Sci. \textbf{109}(28), 11455--11459 (2012)
\else\bibitem{b13} M. Zorzi \textit{et al.}, ``Extra-large letter spacing improves reading in dyslexia,'' \textit{Proc. Nat. Acad. Sci. (PNAS)}, vol. 109, no. 28, pp. 11455--11459, 2012.\fi
\ifspringer\bibitem{b14} Rello, L., Baeza-Yates, R.: Good fonts for dyslexia. In: Proc. ACM SIGACCESS Conf. Comput. Accessibility (ASSETS) (2013)
\else\bibitem{b14} L. Rello and R. Baeza-Yates, ``Good fonts for dyslexia,'' in \textit{Proc. ACM SIGACCESS Conf. Comput. Accessibility (ASSETS)}, 2013.\fi
\ifspringer\bibitem{b15} Kuster, S.M., van Weerdenburg, M., Gompel, M., Bosman, A.: Dyslexie font does not benefit reading in children with or without dyslexia. Ann. Dyslexia \textbf{68}, 25--42 (2018)
\else\bibitem{b15} S. M. Kuster, M. van Weerdenburg, M. Gompel, and A. Bosman, ``Dyslexie font does not benefit reading in children with or without dyslexia,'' \textit{Annals of Dyslexia}, vol. 68, pp. 25--42, 2018.\fi
\ifspringer\bibitem{b16} Nag, S.: Early reading in Kannada: the pace of acquisition of orthographic knowledge and phonemic awareness. J. Res. Reading \textbf{30}(1), 7--22 (2007)
\else\bibitem{b16} S. Nag, ``Early reading in Kannada: The pace of acquisition of orthographic knowledge and phonemic awareness,'' \textit{J. Res. Reading}, vol. 30, no. 1, pp. 7--22, 2007.\fi
\ifspringer\bibitem{b17} Nag, S., Snowling, M.J.: Reading in an alphasyllabary: implications for a language-universal theory of learning to read. Sci. Stud. Reading \textbf{16}(5), 404--423 (2012)
\else\bibitem{b17} S. Nag and M. J. Snowling, ``Reading in an alphasyllabary: Implications for a language-universal theory of learning to read,'' \textit{Sci. Stud. Reading}, vol. 16, no. 5, pp. 404--423, 2012.\fi
\ifspringer\bibitem{b18} Kincaid, J.P., Fishburne, R.P., Rogers, R.L., Chissom, B.S.: Derivation of new readability formulas for Navy enlisted personnel. Naval Tech. Training Command, Rep. 8-75 (1975)
\else\bibitem{b18} J. P. Kincaid, R. P. Fishburne, R. L. Rogers, and B. S. Chissom, ``Derivation of new readability formulas for Navy enlisted personnel,'' Naval Tech. Training Command, Rep. 8-75, 1975.\fi
\ifspringer\bibitem{b19} Xu, W., Callison-Burch, C., Napoles, C.: Problems in current text simplification research: new data can help. Trans. Assoc. Comput. Linguist. \textbf{3}, 283--297 (2015)
\else\bibitem{b19} W. Xu, C. Callison-Burch, and C. Napoles, ``Problems in current text simplification research: New data can help,'' \textit{Trans. Assoc. Comput. Linguist. (TACL)}, vol. 3, pp. 283--297, 2015.\fi
\fi
\ifspringer\bibitem{b20} Micikevicius, P., et al.: Mixed precision training. In: Proc. ICLR (2018)
\else\bibitem{b20} P. Micikevicius \textit{et al.}, ``Mixed precision training,'' in \textit{Proc. ICLR}, 2018.\fi
\ifspringer\bibitem{b21} Khan Academy: Khanmigo: an AI tutor for learners and assistant for teachers (2023), \url{https://khanmigo.ai}
\else\bibitem{b21} Khan Academy, ``Khanmigo: an AI tutor for learners and assistant for teachers,'' 2023. [Online]. Available: khanmigo.ai\fi
\ifspringer\bibitem{b22} Ministry of Electronics and Information Technology, Government of India: BHASHINI: National Language Translation Mission (2022), \url{https://bhashini.gov.in}
\else\bibitem{b22} Ministry of Electronics and Information Technology, Government of India, ``BHASHINI: National Language Translation Mission,'' 2022. [Online]. Available: bhashini.gov.in\fi
\ifshort\else  % cited only by material the six-page build drops
\ifspringer\bibitem{b23} Radford, A., Kim, J.W., Xu, T., Brockman, G., McLeavey, C., Sutskever, I.: Robust speech recognition via large-scale weak supervision. In: Proc. ICML (2023)
\else\bibitem{b23} A. Radford, J. W. Kim, T. Xu, G. Brockman, C. McLeavey, and I. Sutskever, ``Robust speech recognition via large-scale weak supervision,'' in \textit{Proc. ICML}, 2023.\fi
\fi
\ifspringer\bibitem{b25} Robertson, S., Zaragoza, H.: The probabilistic relevance framework: BM25 and beyond. Found. Trends Inf. Retr. \textbf{3}(4), 333--389 (2009)
\else\bibitem{b25} S. Robertson and H. Zaragoza, ``The probabilistic relevance framework: BM25 and beyond,'' \textit{Foundations and Trends in Information Retrieval}, vol.~3, no.~4, pp.~333--389, 2009.\fi
\ifspringer\bibitem{b26} Wilson, E.B.: Probable inference, the law of succession, and statistical inference. J. Amer. Statist. Assoc. \textbf{22}(158), 209--212 (1927)
\else\bibitem{b26} E. B. Wilson, ``Probable inference, the law of succession, and statistical inference,'' \textit{J.~Amer. Statist. Assoc.}, vol.~22, no.~158, pp.~209--212, 1927.\fi
\ifspringer\bibitem{b27} Es, S., James, J., Espinosa-Anke, L., Schockaert, S.: RAGAs: automated evaluation of retrieval augmented generation. In: Proc. EACL: System Demonstrations, pp. 150--158 (2024)
\else\bibitem{b27} S. Es, J. James, L. Espinosa-Anke, and S. Schockaert, ``RAGAs: Automated evaluation of retrieval augmented generation,'' in \textit{Proc. EACL: System Demonstrations}, 2024, pp.~150--158.\fi
\ifspringer\bibitem{b28} Gao, Y., et al.: Retrieval-augmented generation for large language models: a survey. arXiv:2312.10997 (2023)
\else\bibitem{b28} Y. Gao \textit{et al.}, ``Retrieval-augmented generation for large language models: A survey,'' \textit{arXiv preprint arXiv:2312.10997}, 2023.\fi
\ifspringer\bibitem{b29} Ji, Z., et al.: Survey of hallucination in natural language generation. ACM Comput. Surv. \textbf{55}(12), art. 248 (2023)
\else\bibitem{b29} Z. Ji \textit{et al.}, ``Survey of hallucination in natural language generation,'' \textit{ACM Comput. Surv.}, vol.~55, no.~12, art.~248, 2023.\fi
\ifspringer\bibitem{b30} Dept. of School Education and Literacy: UDISE+ 2024--25 report. Ministry of Education, India (2025)
\else\bibitem{b30} Dept. of School Education and Literacy, ``UDISE+ 2024--25 report,'' Ministry of Education, India, 2025.\fi
\ifspringer\bibitem{b24} Registrar General of India: Census of India 2011, Table C-17: bilingualism and trilingualism (2011)
\else\bibitem{b24} Registrar General of India, ``Census of India 2011, Table C-17: bilingualism and trilingualism,'' 2011.\fi
\end{thebibliography}
\endgroup

\end{document}
