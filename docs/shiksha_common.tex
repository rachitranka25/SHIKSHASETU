...
\begin{abstract}
\ifconf
Existing AI tutoring systems depend on cloud infrastructure, operate primarily in English, and are not aligned to a national curriculum. Shiksha Setu answers all three. Its corpus, the embeddings over it, and the whole of retrieval sit on the user's own machine; generation stays a hosted call, and the paper does not claim to be offline. Every number here comes from 153 ingested textbooks spanning classes~1--12, or 42{,}995 indexed passages. A five-configuration ablation over 22 queries in eleven Indian languages, run against an English-medium corpus, is where the cross-lingual gap shows most plainly: a lexical baseline retrieves \emph{nothing at all}, since a Devanagari or Perso-Arabic query shares no tokens with an English index, while dense retrieval alone recovers 11 of 22, query rewriting alone 14, and pooling the two, the deployed configuration, $16\pm1$. A grounding score separates answers written from their retrieved passages from answers that drifted, by 0.360 in cosine similarity over 256 pairs. The threshold shipped before this work admitted 35 of 240 negatives. Half precision halves the embedding footprint to 1{,}083~MB, which is what puts it on a 4~GB machine. Ingestion catches something subtler: legacy Devanagari fonts that render 66\% of Hindi textbooks as Latin gibberish.
\else
...
Neither is therefore the right configuration on its own. Pooling both retrievals scores \textbf{16 of 22}, above either arm, because the union recovers what each misses instead of averaging them. Fig.~\ref{fig:perlang} gives the deployed configuration language by language, as the mean reciprocal rank of its two queries.
