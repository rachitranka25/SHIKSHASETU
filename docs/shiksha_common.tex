...
A student's question traverses an eight-stage pipeline from input to response, six of them mandatory. Fig.~\ref{fig:pipeline} shows those six with their measured latencies on an 8~GB Apple~M1, arranged by the property this paper is about, which is which side of the machine boundary each one runs on.
...
\node[net] (rw) at (52mm, -8mm)   {1\, rewrite to English\\[-1pt]$\sim$0.6\,s};
\node[loc] (em) at (14mm, -16mm)  {2\, embed query\\[-1pt]$\sim$0.1\,s};
\node[loc] (rt) at (14mm, -25mm)  {3--5\, retrieve, score by\\[-1pt]class, retrieve again\\[-1pt]$\sim$0.05\,s};
\node[net] (gn) at (52mm, -33mm)  {6\, generate answer\\[-1pt]$\sim$2.0\,s};
...
\end{tikzpicture}
\caption{The mandatory request path, warm cache, on an 8~GB Apple~M1. The dashed line is the machine boundary. Everything on its left, including the corpus and the whole of retrieval, stays on the machine; the two hosted calls are the only points at which a question leaves it, and the answer crosses back for grounding. Stages~3--5 are drawn as one box because they are three vector operations totalling ${\sim}0.05$\,s. Stage~6 produces a complete answer and is therefore slower than the fixed short completion used for model selection\ifconf\else\ (Table~\ref{tab:latency})\fi. The grounding check carries no latency because we did not measure it separately. The two optional stages, reading support and illustration, are omitted here and add ${\sim}0.5$\,s and ${\sim}5.0$\,s; without them a warm request completes in approximately three seconds.}
\label{fig:pipeline}
\end{figure}
...
The platform is validated by \textbf{474 automated tests}: 381 unit, 73 integration, 10 end-to-end, 9 performance and 1 manual. \textbf{441 pass and 33 are skipped, with zero failures and zero errors.} The skips are declared, not silent. They are tests requiring a loaded model or a service this environment does not run, and the counts here are read from the suite's own JUnit report, not from a summary line.
