...
Each result is reported with the corpus state it was measured against. Grounding used \textbf{153 textbooks}, 138 English and 15 Hindi, 1{,}542 chapters and \textbf{42{,}995 indexed passages}; two more were recovered before the retrieval ablation, which used \textbf{155 textbooks}, 1{,}557 chapters and \textbf{43{,}621 passages}. Every passage in both has an embedding, one lacking it being invisible to retrieval and not otherwise detectable. Reporting a single state would have been tidier and false. All twelve classes appear, from 3 books at class~1 to 34 at class~12.
...
\label{sec:crossling}
Retrieval was evaluated over \textbf{77 queries: seven topics in each of the eleven supported Indian languages}, against the English-medium portion of the corpus, so that language is the only variable moving within a topic. The topics span mathematics, physics, chemistry, biology and geography and were fixed before any was run, each required to have enough English material to answer it; one candidate was discarded for matching four passages, which would have measured the corpus rather than the retriever. The queries are committed with the benchmark, not only their results.
...
Pooling the two scores \textbf{67 of 77}, one query above rewriting alone, with intervals that almost coincide. The per-topic figures show why that is not a claim of dominance: on the eye it scores 7 of 11 where direct embedding scores 9, and on the water cycle 7 against 8. Pooling merges by best score, so a confident wrong passage from one arm displaces a correct one from the other. It is deployed because it was never the worst arm on any topic, a weaker and more accurate reason than this paper previously gave. Fig.~\ref{fig:perlang} gives it per language, as the mean reciprocal rank of that language's seven queries.
...
\textbf{What 77 queries can and cannot establish.} Two conclusions are secure: the lexical baseline's interval reaches only 0.048 and excludes every other configuration, so its failure is structural; and rewriting gains nineteen queries over direct embedding, a separation no reading of the sample size removes. One is not. Pooling scores a single query above rewriting, its interval sits inside rewriting's, and on two topics it is worse than the better arm, so it is reported as what the system deploys rather than as a configuration shown to be better.
...
One query fails in every configuration: Urdu \emph{photosynthesis}. The term (\textit{ziyai talif}, Perso-Arabic in origin) shares no morphology with the English word or with the Sanskrit-derived compounds the other ten languages use, so neither arm has anything to match.
