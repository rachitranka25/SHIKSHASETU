...
The comparison is confined to \emph{architectural properties}. Every cell is a structural fact, a published vendor figure, or \emph{n/d} where none exists, and no row sets a measurement of ours against a number we could not obtain. Commercial products publish neither latency distributions nor retrieval quality nor memory footprints, and estimating them would produce invented numbers set in the typography of measurement.
...
\label{sec:crossling}
Retrieval was evaluated over \textbf{77 queries: seven topics in each of the eleven supported Indian languages}, against the English-medium portion of the corpus, so that language is the only variable moving within a topic. The topics span mathematics, physics, chemistry, biology and geography, were fixed before any was run, and each had to have enough English material to answer it. The queries are committed with the benchmark, not only their results.
...
Pooling the two scores \textbf{67 of 77}, one query above rewriting alone, with intervals that almost coincide. The per-topic figures show why that is not a claim of dominance: on the eye it scores 7 of 11 where direct embedding scores 9, and on the water cycle 7 against 8. Pooling merges by best score, so a confident wrong passage from one arm displaces a correct one from the other. It is deployed because it was never the worst arm on any topic. Of the ten queries it misses, nine place an on-topic passage at rank~2 or~3; one, Urdu \emph{photosynthesis}, finds nothing at all. Fig.~\ref{fig:perlang} gives it per language, as the mean reciprocal rank of that language's seven queries.
...
\textbf{What 77 queries can and cannot establish.} Two conclusions are secure: the lexical baseline's interval reaches only 0.048 and excludes every other configuration, so its failure is structural; and rewriting gains nineteen queries over direct embedding, which no reading of the sample size removes. One is not. Pooling scores a single query above rewriting, its interval sits inside rewriting's, and on two topics it is worse than the better arm, so it is reported as what the system deploys, not as a configuration shown to be better.
...
The actual peak is \textbf{2,094~MB}, not the 1,600~MB predicted\ifconf\else\ (Fig.~\ref{fig:memory})\fi. Loading model weights accounts for only 534~MB (\texttt{safetensors} memory-maps them). The first forward pass adds 1,560~MB as weight pages become resident and attention workspaces are allocated. The system fits within 4~GB with ${\sim}1.4$~GB usable headroom (accounting for ${\sim}500$~MB of operating system overhead), not the 2.4~GB arithmetic implied.
...
\end{axis}
\end{tikzpicture}
\caption{Four grounding estimators over 16 matched and 240 mismatched pairs, with one standard deviation. What separates the populations is the distance between the two bars, not the height of either.}
...
\bibitem{b30} Dept. of School Education and Literacy, ``UDISE+ 2024--25 report,'' Ministry of Education, India, 2025.
\bibitem{b24} Registrar General of India, ``Census of India 2011, Table C-17: bilingualism and trilingualism,'' 2011.
\end{thebibliography}
\endgroup
